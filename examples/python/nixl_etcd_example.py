...
def main():
    ...
    while not (agent1.check_remote_metadata(AGENT2_NAME) and agent2.check_remote_metadata(AGENT1_NAME)):
        time.sleep(0.5)
    ...
    req_src_descs = agent1.get_xfer_descs([(addr1 + 16, req_size, 0)], "DRAM")  # random offset
    req_dst_descs = agent2.get_xfer_descs([(addr2 + dst_offset, req_size, 0)], "DRAM") # random offset
