...
from utils import init_dist, bench, bench_kineto, calc_diff, create_grouped_scores, inplace_unique, per_token_cast_to_fp8, per_token_cast_back  # noqa: E402
...
def test_main(args: argparse.Namespace, num_sms: int,
        ...
        for async_mode in (False, True):
            for current_x in (x_pure_rand, x, x_e4m3):
                for with_topk in (False, True):
                    ...
                    recv_x, recv_topk_idx, recv_topk_weights, recv_num_tokens_per_expert_list, handle, event = buffer.ht_dispatch(**dispatch_args)
                    ...
                    if not with_topk:
                        ...
                        if previous_mode:
                            dispatch_args.update({'previous_event': buffer.capture()})
                        recv_x_cached, _, _, _, _, event = buffer.ht_dispatch(**dispatch_args)
                        event.current_stream_wait() if async_mode else ()
                    ...
                    combined_x, combined_topk_weights, event = buffer.ht_combine(**combine_args)
                    event.current_stream_wait() if async_mode else ()
    ...
    for current_x in (x_e4m3, x):
        ...
        for nvl_chunk_size in range(4, 45, 4):
            for rdma_chunk_size in range(4, 33, 4):
                ...
                t, notify_t = bench_kineto(lambda: buffer.ht_dispatch(**tune_args), ('dispatch', 'notify'))
                if t < best_time:
    ...
    for nvl_chunk_size in range(1, 8, 1):
        for rdma_chunk_size in range(12 if num_nodes == 2 else 8, 33, 4):
            ...
            t, notify_t = bench_kineto(lambda: buffer.ht_combine(**tune_args), ('combine', 'notify'))
            if local_rank == 0:
...
def test_loop(local_rank: int, num_local_ranks: int, args: argparse.Namespace):
    ...
    buffer = nixl_ep.Buffer(rank=rank, low_latency_mode=False, explicitly_destroy=True, group=group, tcp_store_group=tcp_store)
