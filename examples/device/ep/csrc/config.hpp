...
struct Config {
    ...
    size_t get_nvl_buffer_size_hint(size_t hidden_bytes, int num_ranks, int nvl_group_size = NUM_MAX_NVL_PEERS) const {
        ...
        // HT kernels keep fixed-size NVL scratch layouts with NUM_MAX_NVL_PEERS
        // as the stride. nvl_group_size controls active peer mapping, not the
        // maximum scratch lane count needed by the buffer layout.
