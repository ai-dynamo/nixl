...
dispatch(void* packed_recv_x, void* packed_recv_x_scales,
         ...
         uint64_t* recv_pub, int recv_slices,
...
DISPATCH_RECV:
    ...
    // Split each (source rank, local expert) copy across `recv_slices` blocks
    int recv_expert_idx, recv_slice;
    if (recv_slices > 1) {
        recv_expert_idx = sm_id % active_expert_bound;
        recv_slice = sm_id / active_expert_bound;
        // No grid sync below, so an early return is safe
        if (recv_slice >= recv_slices)
            return;
    } else {
        recv_expert_idx = responsible_expert_idx;
        recv_slice = 0;
    }
    if (recv_expert_idx < active_expert_bound) {
        const auto src_rank = recv_expert_idx / num_local_experts;
        const auto local_expert_idx = recv_expert_idx % num_local_experts;
        ...
        if (sub_warp_id == 1 and lane_id == 0 and recv_slice > 0) {
            ...
            shared_num_recv_tokens[warp_group_id] = num_recv_tokens;
            shared_recv_token_begin_idx[warp_group_id] = recv_token_begin_idx;
            const auto packed_range = pack2<int, int64_t>(num_recv_tokens, recv_token_begin_idx);
            recv_range[src_rank] = packed_range;
            if (recv_slices > 1)
                st_release_sys_global(recv_pub + recv_expert_idx, static_cast<uint64_t>(packed_range));
        ...
        const int worker_id = recv_slice * num_warps_per_group + sub_warp_id;
        const int num_workers = recv_slices * num_warps_per_group;
        for (int i = worker_id; i < num_recv_tokens; i += num_workers) {
            // Copy source info
...
void dispatch(void* packed_recv_x, void* packed_recv_x_scales,
    ...
    // One publication slot per expert, after the two counters
    auto recv_pub = reinterpret_cast<uint64_t*>(atomic_finish_counter_per_expert + active_expert_bound);
    EP_HOST_ASSERT(active_expert_bound * sizeof(int) * 2 +
                   active_expert_bound * sizeof(uint64_t) <= NUM_WORKSPACE_BYTES);

    // Let surplus blocks help copy, one slice per `num_warps_per_group` tokens
    int recv_slices = 1;
    if (num_warp_groups == 1 and num_sms > active_expert_bound) {
        const int tokens_per_pair = ceil_div(num_tokens * num_topk, active_expert_bound);
    ...
    if (recv_slices > 1 and (phases & EP_RECV_PHASE))
        CUDA_CHECK(cudaMemsetAsync(recv_pub, 0xFF, active_expert_bound * sizeof(uint64_t), stream));
