...
__global__ void get_dispatch_layout(const topk_idx_t* topk_idx,
                                    ...
                                    int num_tokens, int num_topk, int num_ranks, int num_nvl_ranks, int num_experts) {
    ...
    if (num_tokens_per_rdma_rank != nullptr) {
        EP_DEVICE_ASSERT(num_nvl_ranks > 0 and num_nvl_ranks <= NUM_MAX_NVL_PEERS);
        EP_DEVICE_ASSERT(num_ranks <= num_nvl_ranks or num_ranks % num_nvl_ranks == 0);
    }

    // Count rank statistics
    constexpr int kMaxRDMARanksPerSM = kNumRanksPerSM;
    __shared__ int num_tokens_per_rank_per_thread[kNumThreads][kNumRanksPerSM];
    __shared__ int num_tokens_per_rdma_rank_per_thread[kNumThreads][kMaxRDMARanksPerSM];
    ...
    int rdma_rank_begin_idx = rank_begin_idx / num_nvl_ranks, rdma_rank_end_idx = ceil_div<int>(rank_end_idx, num_nvl_ranks);
    if (rank_begin_idx < rank_end_idx) {
        ...
        #pragma unroll
        for (int i = 0; i < kMaxRDMARanksPerSM; ++ i)
            num_tokens_per_rdma_rank_per_thread[thread_id][i] = 0;
        #pragma unroll
        for (int i = thread_id; i < num_tokens; i += kNumThreads) {
            auto shifted_topk_idx = topk_idx + i * num_topk;
            int is_in_rank[kNumRanksPerSM] = {0}, is_in_rdma_rank[kMaxRDMARanksPerSM] = {0};
            #pragma unroll
            for (int j = 0, expert_idx, rank_idx; j < num_topk; ++j) {
                ...
                if (expert_begin <= expert_idx and expert_idx < expert_end) {
                    ...
                    int rdma_rank_idx = (rank_begin_idx + rank_idx) / num_nvl_ranks - rdma_rank_begin_idx;
                    is_in_rank[rank_idx] ++, is_in_rdma_rank[rdma_rank_idx] ++;
                }
            }
