...
__forceinline__ __device__ int translate_dst_rdma_rank(const int dst_rdma_rank, const int nvl_rank) {
    // With NIXL memory-view API, always use global rank as index
    // (old batch API used RDMA rank for non-low-latency mode, but memory views are indexed by global rank)
...
__forceinline__ __device__ void nixl_barrier(nixl_ep::gpu_nixl_ctx nixl_ctx, int num_channels) {
    ...
    for (int j=0; j<num_channels; j++) {
        for (int i = 0; i < nixl_ctx.num_rdma_ranks; i++) {
            ...
            nixl_status_t status = nixlAtomicAdd<nixl_gpu_level_t::THREAD>(
                1, barrier_mdesc, j, UCP_DEVICE_FLAG_NODELAY, &request_status);
    ...
    if (sm_id == 0) {
        ...
        for (int i = warp_id; i < kNumRDMARanks; i += num_warps) {
            if (i != rdma_rank) {
                ...
                nixlMemDesc src_mdesc{nixl_ctx.local_mvh, 0, src_offset};
                nixlMemDesc dst_mdesc{nixl_ctx.remote_mvh, (size_t)translated_dst, dst_offset};
                nixl_status_t status = nixlPut<nixl_gpu_level_t::WARP>(
                    src_mdesc, dst_mdesc, msg_size, 0, UCP_DEVICE_FLAG_NODELAY);
                EP_DEVICE_ASSERT(status == NIXL_IN_PROG);
            } else {
    ...
    if (warp_role == WarpRole::kRDMASender) {
        ...
        for (int dst_rdma_rank = warp_id; dst_rdma_rank < kNumRDMARanks; dst_rdma_rank += kNumDispatchRDMASenderWarps) {
            ...
            if (dst_rdma_rank != rdma_rank) {
                ...
                nixlMemDesc src_mdesc{nixl_ctx.local_mvh, 0, src_offset};
                nixlMemDesc dst_mdesc{nixl_ctx.remote_mvh, (size_t)translated_rank, dst_offset};
                EP_DEVICE_ASSERT(nixlPut<nixl_gpu_level_t::WARP>(
                                     src_mdesc, dst_mdesc, msg_size, channel_id, UCP_DEVICE_FLAG_NODELAY) ==
                                 NIXL_IN_PROG);
            }
        }
        ...
        while (__any_sync(0xffffffff, num_tokens_to_send > 0)) {
            ...
            for (int i = 0, synced_num_tokens_to_send; i < kNumRDMARanks; ++i) {
                ...
                if (dst_rdma_rank != rdma_rank) {
                    ...
                    nixlMemDesc dst_mdesc{nixl_ctx.remote_mvh, (size_t)translated_dst, dst_offset};
                    EP_DEVICE_ASSERT(nixlPut<nixl_gpu_level_t::WARP>(
                                         src_mdesc, dst_mdesc, num_bytes_per_msg, channel_id, UCP_DEVICE_FLAG_NODELAY) ==
                                     NIXL_IN_PROG);

                    // NOTE: no put-completion wait -- relying on same-channel ordering
                    // If this causes data corruption / timeouts, the put-completion wait is needed.
                    // Uncomment below to restore the wait:
                    // nixl_status_t put_status = NIXL_IN_PROG;
                    // while ((put_status = nixlGpuGetXferStatus<nixl_gpu_level_t::WARP>(rdma_put_xfer_status)) == NIXL_IN_PROG)
                    //     ;
                ...
                if (lane_id == dst_rdma_rank) {
                    ...
                    if (dst_rdma_rank == rdma_rank) {
                        ...
                        nixlMemDesc tail_mdesc{nixl_ctx.remote_mvh, (size_t)translated_dst_tail, tail_counter_offset};
                        EP_DEVICE_ASSERT(nixlAtomicAdd<nixl_gpu_level_t::THREAD>(
                            num_tokens_to_issue, tail_mdesc, channel_id, UCP_DEVICE_FLAG_NODELAY) ==
                        NIXL_IN_PROG);
                    }
                }
                __syncwarp();
        ...
        while (true) {
            ...
            if (min_head != std::numeric_limits<int>::max() and min_head >= last_head + num_max_rdma_chunked_send_tokens and
                ...
                if(lane_id == rdma_rank){
                    ...
                    nixlMemDesc head_mdesc{nixl_ctx.remote_mvh, (size_t)translated_dst_head, head_counter_offset};
                    nixl_status_t status = nixlAtomicAdd<nixl_gpu_level_t::THREAD>(
                        min_head - last_head, head_mdesc, channel_id, UCP_DEVICE_FLAG_NODELAY);
            ...
            for (int token_start_idx = 0; token_start_idx < num_tokens_to_combine; token_start_idx += num_max_rdma_chunked_send_tokens) {
                ...
                if (sub_warp_id == kNumWarpsPerForwarder - 1) {
                    if (dst_rdma_rank != rdma_rank) {
                        ...
                        nixlMemDesc src_mdesc_comb{nixl_ctx.local_mvh, 0, nixl_ctx.offset_get(src_ptr)};
                        nixlMemDesc dst_mdesc_comb{nixl_ctx.remote_mvh, (size_t)translated_dst_comb, nixl_ctx.offset_get(dst_ptr)};
                        EP_DEVICE_ASSERT(nixlPut<nixl_gpu_level_t::WARP>(
                                             src_mdesc_comb, dst_mdesc_comb, num_bytes_per_msg, channel_id, UCP_DEVICE_FLAG_NODELAY) ==
                                         NIXL_IN_PROG);
                    } else {
                        memory_fence();
                    }
                    ...
                    if (elect_one_sync()) {
                        ...
                        if(dst_rdma_rank == rdma_rank){
                            ...
                            nixlMemDesc tail_mdesc_ct{nixl_ctx.remote_mvh, (size_t)translated_dst_ct, nixl_ctx.offset_get(tail_ptr)};
                            EP_DEVICE_ASSERT(nixlAtomicAdd<nixl_gpu_level_t::THREAD>(
                                                 num_chunked_tokens, tail_mdesc_ct, channel_id, UCP_DEVICE_FLAG_NODELAY) ==
                                             NIXL_IN_PROG);
                        }
                    }
                }
                ...
                if (not is_forwarder_sm) {
                    ...
                    if (min_head != std::numeric_limits<int>::max() and min_head >= last_rdma_head + num_max_rdma_chunked_send_tokens and
                        ...
                        if (dst_rdma_rank == rdma_rank) {
                            ...
                            nixlMemDesc head_mdesc_ch{nixl_ctx.remote_mvh, (size_t)translated_dst_ch, head_counter_offset};
                            EP_DEVICE_ASSERT(nixlAtomicAdd<nixl_gpu_level_t::THREAD>(min_head - last_rdma_head, head_mdesc_ch, channel_id, UCP_DEVICE_FLAG_NODELAY) == NIXL_IN_PROG);
                        }
