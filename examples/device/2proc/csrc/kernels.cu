...
__global__ void
post_write_and_signal_kernel_thread(uintptr_t data_req_handles_ptr,
                                    int data_req_count,
    ...
    if (blockIdx.x == 0 && threadIdx.x == 0) {
        ...
        auto *data_req_handles = reinterpret_cast<const uintptr_t *>(data_req_handles_ptr);
        nixlGpuXferReqH data_req = reinterpret_cast<nixlGpuXferReqH>(data_req_handles[0]);
        nixlGpuXferReqH signal_req = reinterpret_cast<nixlGpuXferReqH>(signal_req_handle);
        for (int i = 0; i < pipeline; ++i) {
            nixlGpuPostSingleWriteXferReq<nixl_gpu_level_t::THREAD>(
                data_req, 0, 0, 0, size, 0, true);
        }
        nixlGpuPostSignalXferReq<nixl_gpu_level_t::THREAD>(signal_req, 0, signal_inc, 0, 0, true);
    }
}

__global__ void
post_write_and_signal_kernel_warp(uintptr_t data_req_handles_ptr,
                                  int data_req_count,
                                  uintptr_t signal_req_handle,
                                  uint64_t *signal_ptr,
                                  size_t size,
    ...
    if (blockIdx.x == 0 && threadIdx.x < WARP_SIZE) {
        ...
        nixlGpuXferReqH data_req = reinterpret_cast<nixlGpuXferReqH>(data_req_handles[handle_idx]);
        nixlGpuXferReqH signal_req = reinterpret_cast<nixlGpuXferReqH>(signal_req_handle);
        for (int i = 0; i < pipeline; ++i) {
            nixlGpuPostSingleWriteXferReq<nixl_gpu_level_t::WARP>(data_req, 0, 0, 0, size, 0, true);
        }
        if (lane == 0) {
            nixlGpuPostSignalXferReq<nixl_gpu_level_t::WARP>(signal_req, 0, signal_inc, 0, 0, true);
        }
    }
}
