...
1. **Version Modification with Build ID**
   ```bash
   # Automatically adds build ID if CI_BUILD_NUMBER is set
...
2. **UV Build with Ninja Control**
   ```bash
   # Control ninja parallelism to prevent OOM
   export NINJA="ninja -j${NPROC:-4}"
   export MESON_BUILD_PARALLEL_LEVEL=${NPROC:-4}
...
4. **UCX Plugin Integration**
   ...
   ./contrib/wheel_add_ucx_plugins.py \
     ...
     --nixl-plugins-dir $NIXL_PLUGINS_DIR \
     $TMP_DIR/dist/*.whl
   ```
