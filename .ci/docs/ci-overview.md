...
> **Note on Jenkins jobs:** `proj-jjb.yaml` defines 12 Jenkins jobs. Seven are in
> the PR CI flow — `nixl-ci-dispatcher` plus the 6 jobs it fans out to. The
> other 5 (`build-container`, `build-wheel-nightly`,
> `build-wheel-release-poller`, `build-llm-container`,
> `test-llm-container`) are entirely standalone — they run only on their own
...
- **What it does:** Reuses the per-PR wheel build path (`contrib/build-container.sh` + `Dockerfile.manylinux`), adds UCX wiring, and publishes wheels to `sw-nbu-swx-nixl-pypi-local` under `<PUBLISH_DIR|verification>/nixl-<nixl-sha8>-ucx-<ucx-sha8>/`. With `PUBLISH_DIR` empty (the default, and what the nightly cron uses) wheels land under `verification/`; the poller passes `release/<ver>`. `BUILD_UCX_SPCX_PLUGIN` + `UCX_SPCX_PLUGIN_REF` opt in to bundling the UCX spcx external plugin, which needs a source ref carrying the spcx support in `contrib/`.
