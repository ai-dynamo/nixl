...
- **UCX spcx plugin compile check:** `pipeline_start` triggers `nixl-ci-compile-ucx-plugin` and fails the job if it fails, before any wheel is built. That job lives in the closed-source plugin repo (its definition, matrix and build scripts) but runs on this Jenkins instance so it can write to the shared PVC. It builds both arches against the same `wheel_base` image this job uses and stages the module under `SPCX_STAGING_ROOT/<build>/<arch>/`; the Build Wheel step passes that to `build-container.sh --ucx-spcx-plugin-install`, and `wheel_add_ucx_plugins.py` bundles it like any other UCX module. The plugin is never cloned or built here — this repo is public and everything in a PR's checkout is PR-editable, so the plugin's read credential and source stay on the trusted side; only the built module and a pass/fail crosses back. Which plugin ref gets built is that job's own default, owned by the plugin developers, so a plugin bump needs no change here.
...
- **What it does:** Reuses the per-PR wheel build path (`contrib/build-container.sh` + `Dockerfile.manylinux`) and publishes wheels to `sw-nbu-swx-nixl-pypi-local`. With `PUBLISH_DIR` empty (the default, and what the nightly cron uses) wheels land under `verification/g<nixl-sha8>.ucx<ucx-sha8>/` — the long-standing schema the `build-llm-container` verification pipeline consumes; the poller and manual release runs pass `release/<ver>`, which co-locates cu12/cu13 under `release/<ver>/<nixl-sha8>/` (unkeyed to UCX). `CUDA_MAJOR` selects the CUDA line to build: `13` (default) passes no base-image flags to `build-container.sh` and relies on its own defaults; `12` passes the pinned CUDA 12 base image/tag from the matrix env. The Infinia DDN plugin is always bundled (`--build-infinia`, unconditional — not a job parameter) and the source ref's `contrib/build-container.sh` must carry the flag and pin its version; the UCX spcx plugin is compiled by `nixl-ci-compile-ucx-plugin` instead and passed in with `--ucx-spcx-plugin-install`. The resolved build options (base image, UCX ref/sha, plugin flags, etc.) are written to `build_options.env` via `--build-options-file`; Publish reads it and attaches `UCX_REF`, `UCX_SHA`, `CUDA_VERSION`, and (when built) `UCX_SPCX_PLUGIN_REF`/`INFINIA_LIBS_IMAGE` as Artifactory properties on each uploaded wheel (skipped if the built ref's `build-container.sh` predates `--build-options-file`).
