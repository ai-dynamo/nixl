#!/usr/bin/env bash
#
# Trigger the generic `lyris-exec` pipeline, wait for it, and report the result.
#
# Usage:
...
#
# Exit code: 0 = all tests passed, 1 = a test failed, 90 = infra failure
# (allocation/runner/queue) after retries. The job summary is downloaded to
# ./lyris-artifacts/summary.txt.
set -uo pipefail
...
while [ $# -gt 0 ]; do
  case "$1" in
    ...
    --allow-fail)     allow_fail=$2;     shift 2 ;;   # test names to run but not gate on
...
while :; do
  ...
  # 3. download the lyris-exec job's summary
  ...
  api_get "/jobs/$job_id/artifacts/results/summary.txt" > lyris-artifacts/summary.txt || true

  # 4. infra failure -> retry then give up (90); otherwise gate on the test count
  if [ "$status" = canceled ] || grep -q '^INFRA_FAILURE' lyris-artifacts/summary.txt 2>/dev/null; then
    if [ "$attempt" -le "$retries" ]; then echo "infra failure, retrying"; continue; fi
    echo "infra failure after $retries retries"; exit 90
  fi
  test_fails=$(awk -F= '/^TEST_FAILS=/{print $2}' lyris-artifacts/summary.txt)
  [ "${test_fails:-1}" = 0 ] && exit 0 || exit 1
done
