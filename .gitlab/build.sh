...
# nixl_ep build (BUILD_NIXL_EP=true) needs DOCA GPUNetIO dev headers because
# UCX device code pulls doca_gpunetio_*.cuh. When PRE_INSTALLED_ENV is set the
# apt bootstrap that would have installed these packages is skipped, so install
# DOCA GPUNetIO dev packages here on demand for this nixl_ep build only.
