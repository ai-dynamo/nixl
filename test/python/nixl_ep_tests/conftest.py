...
REPO_ROOT = os.path.dirname(os.path.dirname(os.path.dirname(TESTS_DIR)))  # Go up to repo root
NIXL_EP_DIR = os.path.join(REPO_ROOT, "examples", "device", "ep")
sys.path.insert(0, NIXL_EP_DIR)

# Import results reporter
from utils.results_reporter import ResultsReporter, get_reporter, init_reporter
...
def sample_tensors(buffer_params):
    ...
    topk_idx = torch.randint(0, num_experts, (num_tokens, 2), dtype=torch.int64, device='cuda')
...
def record_result(results_reporter):
    ...
    """
    def _record(test_name: str, category: str, metric: str, value: float, 
                unit: str = '', params: dict = None):
        results_reporter.add_result(
            test_name=test_name,
            category=category,
            metric=metric,
