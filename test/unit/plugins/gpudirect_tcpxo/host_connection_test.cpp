...
namespace {
    ...
    protected:
        ...
        SetUp() override {
            ...
            initiator_ = std::make_unique<HostConnection>(kInitiatorAgentName,
                                                          *handle_or_status,
    ...
    TEST_F(HostConnectionTest, DisconnectionWorks) {
        ...
        // speed up this test
        EXPECT_TRUE(initiator_control_channel_->Disconnect(initiator_->remote_handle()).ok());
        initiator_.reset();

        ASSERT_TRUE(disconnected.WaitForNotificationWithTimeout(test::kHeartbeatTimeout +
                                                                absl::Seconds(1)));
    }
    ...
    TEST_F(HostConnectionTest, SendNotification) {
        ...
        EXPECT_TRUE(initiator_->SendNotification(kExpectedMessage).ok());
    ...
    TEST_F(HostConnectionTest, XferLifecycle) {
        ...
        EXPECT_EQ(initiator_->PrepXfer(NIXL_WRITE, {}, handle), absl::OkStatus());
        ASSERT_NE(handle, nullptr);

        EXPECT_EQ(initiator_->PostXfer(*handle), absl::OkStatus());
        EXPECT_EQ(initiator_->CheckXfer(*handle), NIXL_SUCCESS);

        EXPECT_EQ(initiator_->ReleaseReqH(*handle), NIXL_SUCCESS);

        // Verify operations fail properly for an untracked/invalid request handle
        nixlTcpxoBackendReqH dummy_handle(NIXL_WRITE, *initiator_, {});
        EXPECT_EQ(initiator_->PostXfer(dummy_handle).code(), absl::StatusCode::kNotFound);
        EXPECT_EQ(initiator_->CheckXfer(dummy_handle), NIXL_ERR_BACKEND);
    }
