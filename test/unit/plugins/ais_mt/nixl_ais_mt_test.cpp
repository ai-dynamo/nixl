...
// Path-mode smoke target. The harness creates this file (and a "<path>.b"
// sibling) itself, so it must name a regular file on a writable filesystem --
// never a raw block device. Point it at AIS-backed storage to exercise the
// hipFile fast path:
