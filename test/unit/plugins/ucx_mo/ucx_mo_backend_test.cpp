...
void allocateBuffer(nixl_mem_t mem_type, int dev_id, size_t len, void* &addr)
{
    ...
    case VRAM_SEG:{
        ...
        checkCudaError(hipSetDevice(dev_id), "Failed to set device");
        checkCudaError(hipMalloc(&addr, len), "Failed to allocate CUDA buffer 0");
...
void releaseBuffer(nixl_mem_t mem_type, int dev_id, void* &addr)
{
    ...
    case VRAM_SEG:
        checkCudaError(hipSetDevice(dev_id), "Failed to set device");
        checkCudaError(hipFree(addr), "Failed to allocate CUDA buffer 0");
        break;
#endif
    default:
        std::cout << "Unsupported memory type!" << std::endl;
...
void doMemset(nixl_mem_t mem_type, int dev_id, void *addr, char byte, size_t len)
{
    ...
    case VRAM_SEG:
        checkCudaError(hipSetDevice(dev_id), "Failed to set device");
        checkCudaError(hipMemset(addr, byte, len), "Failed to memset");
        break;
#endif
    default:
        std::cout << "Unsupported memory type!" << std::endl;
...
void *getValidationPtr(nixl_mem_t mem_type, void *addr, size_t len)
{
    ...
    case VRAM_SEG: {
        ...
        checkCudaError(hipMemcpy(ptr, addr, len, hipMemcpyDeviceToHost), "Failed to memcpy");
        return ptr;
    }
#endif
    default:
...
#ifdef HAVE_CUDA
    int n_vram_dev;
    checkCudaError(hipGetDeviceCount(&n_vram_dev), "hipGetDeviceCount failed.");
