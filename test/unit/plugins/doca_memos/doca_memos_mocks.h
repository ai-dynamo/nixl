...
struct doca_kv_nvme_kernel_dev {};

struct doca_kv_nvme_kernel_dev_create_attr {};

struct doca_kv_nvme_kernel_io {};
...
// so the *_as_task helpers are simple pointer casts.
struct doca_task {
...
struct doca_kv_io_task_store : doca_task {};

struct doca_kv_io_task_retrieve : doca_task {};

struct doca_kv_io_task_exist : doca_task {};
...
public:
    ...
    // Devices no longer start and stop; they bind to a running I/O context instead,
    // so these replace kvdev_start_result / kvdev_stop_result.
    doca_error_t bind_dev_result = DOCA_SUCCESS;
    doca_error_t unbind_dev_result = DOCA_SUCCESS;
    ...
    int task_submit_ex_call_count = 0;
    int ctx_flush_tasks_call_count = 0;
    ...
    // The public API registers callbacks independently for each operation.
    doca_kv_io_task_store_completion_cb_t store_completion_cb = nullptr;
    doca_kv_io_task_store_completion_cb_t store_error_cb = nullptr;
    doca_kv_io_task_retrieve_completion_cb_t retrieve_completion_cb = nullptr;
    doca_kv_io_task_retrieve_completion_cb_t retrieve_error_cb = nullptr;
    doca_kv_io_task_exist_completion_cb_t exist_completion_cb = nullptr;
    doca_kv_io_task_exist_completion_cb_t exist_error_cb = nullptr;
    ...
    std::vector<uint32_t> submitted_task_flags;
    ...
    reset() {
        ...
        bind_dev_result = DOCA_SUCCESS;
        unbind_dev_result = DOCA_SUCCESS;
        ...
        task_submit_ex_call_count = 0;
        ctx_flush_tasks_call_count = 0;
        task_submit_fail_error = DOCA_ERROR_FULL;
        force_task_error = false;
        forced_task_error_code = DOCA_ERROR_NOT_FOUND;

        store_completion_cb = nullptr;
        store_error_cb = nullptr;
        retrieve_completion_cb = nullptr;
        retrieve_error_cb = nullptr;
        exist_completion_cb = nullptr;
        exist_error_cb = nullptr;

        kv_store.clear();
        submitted_tasks.clear();
        submitted_task_user_data.clear();
        submitted_task_flags.clear();
