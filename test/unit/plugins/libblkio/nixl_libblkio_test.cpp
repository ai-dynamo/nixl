...
void
print_phase_header(const std::string &phase_name) {
    std::cout << "\n" << line_str << std::endl;
...
bool
safe_create_file(const std::string &path, size_t size_mb) {
...
bool
safe_execute_command(const std::string &executable, const std::vector<std::string> &args) {
...
std::string
safe_execute_command_with_output(const std::string &executable,
...
bool
setup_test_environment(TestConfig &config) {
    // Validate test_files_dir to prevent path traversal
    if (config.test_files_dir.empty() || config.test_files_dir.find("..") != std::string::npos ||
...
void
cleanup_test_environment(const TestConfig &config) {
...
bool
test_backend_creation(const TestConfig &config) {
    ...
    try {
        ...
        std::cout << "  - Direct I/O: " << (config.enable_direct_io ? "enabled" : "disabled")
                  << std::endl;
        std::cout << "  - I/O Polling: " << (config.enable_io_polling ? "enabled" : "disabled")
                  << std::endl;
...
bool
test_invalid_api_type(const TestConfig &config) {
...
bool
test_memory_registration(const TestConfig &config) {
...
bool
test_basic_io(const TestConfig &config) {
    ...
    try {
        ...
        double write_throughput =
            (config.transfer_size / (1024.0 * 1024.0)) / (write_time / 1000000.0);
        ...
        std::cout << "  - Throughput: " << std::fixed << std::setprecision(2) << write_throughput
                  << " MB/s" << std::endl;
        ...
        double read_throughput =
            (config.transfer_size / (1024.0 * 1024.0)) / (read_time / 1000000.0);

        std::cout << "✓ Read operation completed" << std::endl;
        std::cout << "  - Size: " << config.transfer_size / 1024 << " KB" << std::endl;
        std::cout << "  - Time: " << format_duration(read_time) << std::endl;
        std::cout << "  - Throughput: " << std::fixed << std::setprecision(2) << read_throughput
                  << " MB/s" << std::endl;
...
print_usage(const char *program_name) {
    ...
    std::cout << "  -d, --device <path>     Device path (overridden by NIXL_LIBBLKIO_TEST_DEVICE)"
              << std::endl;
    std::cout << "  -t, --transfers <num>   Number of transfers (default: " << default_num_transfers
              << ")" << std::endl;
    std::cout << "  -s, --size <bytes>      Transfer size in bytes (default: "
    ...
    std::cout << "  --test-dir <path>       Test files directory (default: "
...
main(int argc, char *argv[]) {
    ...
    std::cout << "  Device: " << (config.device_path.empty() ? "<auto-setup>" : config.device_path)
              << std::endl;
    ...
    if (!skip_setup && config.device_path.empty()) {
        ...
        if (!setup_test_environment(config)) {
            ...
            std::cerr << "Note: This test requires root privileges for loop device setup"
                      << std::endl;
            std::cerr << "You can skip setup with --no-setup if you have already set up a device"
                      << std::endl;
            return 1;
        }
