...
public:
    ...
    ~testHndlIterator() {
        ...
        CHECK_NIXL_ERROR(set, "Handler was not released");
    }
    ...
    void unsetHandle() {
        CHECK_NIXL_ERROR(!set, "Handler was not set");
        set = false;
    }
...
createEngine(std::string name, bool p_thread) {
    ...
    CHECK_NIXL_ERROR(ucx->getInitErr(), "Failed to initialize worker1");
...
allocateWrongGPUTest(nixlUcxEngine *ucx, int dev_id) {
    ...
    CHECK_NIXL_ERROR_AGENT(ret, "Failed to register memory", "test");
...
allocateAndRegister(nixlUcxEngine *ucx,
    ...
    CHECK_NIXL_ERROR(ret, "Failed to register memory");
}
...
performTransfer(nixlUcxEngine *ucx1,
    ...
    if (ret3 == NIXL_SUCCESS) {
        ...
        while(ret3 == NIXL_IN_PROG) {
            ...
            CHECK_NIXL_ERROR(!((ret3 == NIXL_SUCCESS) || (ret3 == NIXL_IN_PROG)),
                             "Failed to check xfer");
        }
    }
    ...
    if(use_notif) {
        ...
        while(ret2 == 0){
            ...
            CHECK_NIXL_ERROR(ret3, "Failed to get notifs");
        }

        CHECK_NIXL_ERROR((ret2 != 1), "Incorrect number of target notifs");
...
test_intra_agent_transfer(bool p_thread, nixlUcxEngine *ucx, nixl_mem_t mem_type) {
    ...
    CHECK_NIXL_ERROR(!ucx->supportsLocal(), "Failed to get conn info");
...
test_inter_agent_transfer(bool p_thread,
    ...
    for(int k = 0; k < iter; k++) {
        ...
        while(ret == 0){
            ...
            CHECK_NIXL_ERROR(ret2, "Failed to get notifs");
        }
