...
int main(int argc, char *argv[]) {
	...
	nixl_reg_dlist_t  dram_for_doca(VRAM_SEG);
	nixlXferReqH      *treq;
	...
	if (argc < 5) {
		...
		std::cout <<"<Role> <Peer IP> <Peer Port> <stream mode: attached or pool>"
				  << std::endl;
		exit(-1);
	}
	...
	if (role == target) {
		...
		assert(serdes->addBuf("BaseAddress", &data_address_ptr, sizeof(uintptr_t)) == NIXL_SUCCESS);
		...
		std::cout 	<< " data_address_ptr " << data_address_ptr
					<< " data_address " << (uintptr_t)data_address
					<< " buf_size " << buf_size
					<< " buf_num " << buf_num
					<< std::endl;
		...
		for (const auto &notif : notifs[target]) {
			...
			assert(remote_serdes->getBuf("BaseAddress", &data_address_ptr, sizeof(uintptr_t)) == NIXL_SUCCESS);
			assert(remote_serdes->getBuf("BufferSize", &buf_size, sizeof(size_t)) == NIXL_SUCCESS);
			assert(remote_serdes->getBuf("BufferTransfer", &buf_num, sizeof(uint32_t)) == NIXL_SUCCESS);
		...
		nixl_xfer_dlist_t dram_target_doca(VRAM_SEG);
		for (uint32_t i = 0; i < buf_num; i++) {
			buf[i].addr  = (uintptr_t)(data_address_ptr + (i * buf_size));
			buf[i].len   = buf_size;
			buf[i].devId = 0;
			dram_target_doca.addDesc(buf[i]);
		}
