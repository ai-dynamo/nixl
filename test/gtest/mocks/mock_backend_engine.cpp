...
MockBackendEngine::~MockBackendEngine() {}
...
nixl_status_t MockBackendEngine::registerMem(const nixlBlobDesc &mem, const nixl_mem_t &nixl_mem,
                                             nixlBackendMD *&out) {
  sharedState++;
  return gmock_backend_engine->registerMem(mem, nixl_mem, out);
}

nixl_status_t MockBackendEngine::deregisterMem(nixlBackendMD *meta) {
  sharedState++;
  return gmock_backend_engine->deregisterMem(meta);
}

nixl_status_t MockBackendEngine::connect(const std::string &remote_agent) {
  sharedState++;
  return gmock_backend_engine->connect(remote_agent);
}

nixl_status_t MockBackendEngine::disconnect(const std::string &remote_agent) {
  sharedState++;
  return gmock_backend_engine->disconnect(remote_agent);
}

nixl_status_t MockBackendEngine::unloadMD(nixlBackendMD *input) {
  sharedState++;
  return gmock_backend_engine->unloadMD(input);
}

nixl_status_t MockBackendEngine::prepXfer(const nixl_xfer_op_t &operation,
...
nixl_status_t MockBackendEngine::postXfer(const nixl_xfer_op_t &operation,
                                          const nixl_meta_dlist_t &local,
                                          const nixl_meta_dlist_t &remote,
                                          const std::string &remote_agent,
                                          nixlBackendReqH *&handle,
...
nixl_status_t MockBackendEngine::checkXfer(nixlBackendReqH *handle) const {
  assert(sharedState > 0);
  return gmock_backend_engine->checkXfer(handle);
}

nixl_status_t MockBackendEngine::releaseReqH(nixlBackendReqH *handle) const {
  assert(sharedState > 0);
  return gmock_backend_engine->releaseReqH(handle);
}

nixl_status_t MockBackendEngine::loadRemoteConnInfo(const std::string &remote_agent,
                                                    const std::string &remote_conn_info) {
  sharedState++;
  return gmock_backend_engine->loadRemoteConnInfo(remote_agent, remote_conn_info);
}

nixl_status_t MockBackendEngine::loadRemoteMD(const nixlBlobDesc &input,
...
nixl_status_t MockBackendEngine::loadLocalMD(nixlBackendMD *input,
                                             nixlBackendMD *&output) {
  sharedState++;
  return gmock_backend_engine->loadLocalMD(input, output);
}

nixl_status_t MockBackendEngine::getNotifs(notif_list_t &notif_list) {
  sharedState++;
  return gmock_backend_engine->getNotifs(notif_list);
}

nixl_status_t MockBackendEngine::genNotif(const std::string &remote_agent,
                                          const std::string &msg) const {
  assert(sharedState > 0);
  return gmock_backend_engine->genNotif(remote_agent, msg);
}

int MockBackendEngine::progress() {
  sharedState++;
