...
class UcxTestFixture : public testing::Test {
    class Agent {
        struct MemDesc {
            MemDesc() : m_dlist(DRAM_SEG), m_desc(), m_data(m_data_size) {}
    ...
    public:
        ...
        nixl_status_t wait_for_completion(nixlXferReqH* req_handle);
        nixl_status_t wait_for_notif(const std::string& expectedNotif);
        void fillData();
...
void UcxTestFixture::Agent::init(const std::string& name) {
...
void UcxTestFixture::Agent::destroy() {
    m_MetaRemote.clear();
    m_priv->deregisterMem(m_mem.m_dlist, &m_mem.m_params);
    m_priv.reset();
}

void UcxTestFixture::Agent::fillRegList(nixl_xfer_dlist_t& dlist,
                                        nixlBasicDesc& desc) const {
    nixl::fillRegList(dlist, desc, m_mem.m_data);
}

std::string UcxTestFixture::Agent::getLocalMD() const {
    std::string meta;
    EXPECT_EQ(NIXL_SUCCESS, m_priv->getLocalMD(meta));
    return meta;
}

void UcxTestFixture::Agent::loadRemoteMD(const std::string& remote_name) {
    EXPECT_EQ(NIXL_SUCCESS, m_priv->loadRemoteMD(remote_name, m_MetaRemote));
}

nixl_status_t UcxTestFixture::Agent::createXferReq(const nixl_xfer_op_t& op,
                                nixl_xfer_dlist_t& sReq_descs,
                                nixl_xfer_dlist_t& rReq_descs,
                                nixlXferReqH*& req_handle) const {
...
nixl_status_t
UcxTestFixture::Agent::postXferReq(nixlXferReqH* req_handle) const {
    return m_priv->postXferReq(req_handle);
}

nixl_status_t
UcxTestFixture::Agent::wait_for_completion(nixlXferReqH* req_handle) {
...
nixl_status_t
UcxTestFixture::Agent::wait_for_notif(const std::string& expectedNotif) {
...
void UcxTestFixture::Agent::fillData() {
    m_mem.fillData();
}

bool UcxTestFixture::Agent::dataCmp(const UcxTestFixture::Agent& other) const {
    return m_mem.m_data == other.m_mem.m_data;
}

UcxTestFixture::UcxTestFixture() {
...
template<UcxTestFixture::TestType test_type, enum nixl_xfer_op_t op>
void UcxTestFixture::testXfer() {
    ...
    for (size_t i = 0; i < numIter<test_type>(); ++i) {
        ...
        nixl_status_t status     = m_Initiator.wait_for_completion(req_handle);

        if (isFailure<test_type>(i)) {
            EXPECT_EQ(NIXL_ERR_REMOTE_DISCONNECT, status);
        } else {
            EXPECT_EQ(NIXL_SUCCESS, status);
            EXPECT_EQ(NIXL_SUCCESS, m_Target.wait_for_notif("notification"));
...
template<UcxTestFixture::TestType test_type>
bool UcxTestFixture::isFailure(size_t iter) {
    switch (test_type) {
...
template<UcxTestFixture::TestType test_type> size_t UcxTestFixture::numIter() {
    return (test_type == TestType::XFER_THEN_FAIL) ? 2 : 1;
}

void UcxTestFixture::exchangeMetaData() {
    m_Initiator.loadRemoteMD(m_Target.getLocalMD());
    m_Target.loadRemoteMD(m_Initiator.getLocalMD());
}

nixlXferReqH*
UcxTestFixture::postXfer(enum nixl_xfer_op_t op, bool target_failure) {
