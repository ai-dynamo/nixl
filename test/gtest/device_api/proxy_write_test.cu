...
public:
    ...
    submit(const nixlBackendProxySubmission &submission,
        ...
        request = nixlBackendProxyRequest{next_token_++, submission.channelId};
        pending_.insert(request.token);
        token_channel_[request.token] = submission.channelId;
...
TEST_F(ProxyDeviceApiTest, CompletionNotVisibleUntilPublished) {
    ...
    ASSERT_EQ(
        cudaMemcpy(
            &ring, runtime.deviceChannelViews()[0].workRing, sizeof(ring), cudaMemcpyDeviceToHost),
        cudaSuccess);
    ...
    EXPECT_EQ(deviceGet(d_put_status), NIXL_IN_PROG);
    EXPECT_EQ(deviceGet(d_poll_status), NIXL_SUCCESS);
    EXPECT_EQ(deviceGet(ring.consumerIdx), 1u);
...
TEST_F(ProxyDeviceApiTest, RingSlotsAreReusedAfterWraparound) {
    ...
    constexpr uint32_t kOps = default_proxy_ring_depth + 3;
    ...
    ASSERT_EQ(
        cudaMemcpy(&producer_idx, ring.producerIdx, sizeof(producer_idx), cudaMemcpyDeviceToHost),
        cudaSuccess);
    EXPECT_EQ(producer_idx, kOps);
    EXPECT_EQ(deviceGet(ring.consumerIdx), kOps);
...
TEST_F(ProxyDeviceApiTest, RingOverflowReturnsBackendErrorOnShutdown) {
    ...
    constexpr uint32_t kBurstOps = default_proxy_ring_depth + 1;
    ...
    ASSERT_EQ(
        ...
        cudaSuccess);
    for (uint32_t i = 0; i < default_proxy_ring_depth; ++i) {
