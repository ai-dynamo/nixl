...
private:
    ...
    malloc(size_t count, nixl_mem_t mem_type) {
        ...
        case VRAM_SEG:
            ...
            if (err != cudaSuccess) {
                throw std::runtime_error(std::string("memTypeArray: cudaMalloc failed: ") +
        ...
        default:
            throw std::runtime_error(std::string("memTypeArray: unsupported memory type: ") +
                                     std::to_string(mem_type));
        }

        if (ptr == nullptr) {
            throw std::runtime_error("memTypeArray: failed to allocate memory");
        }
        return ptr;
    }
    ...
    copy(void *dst, const void *src, size_t count, cudaMemcpyKind kind) const {
        if (count > count_) {
            throw std::out_of_range("memTypeArray: copy count exceeds array size");
        }
        ...
        if (err != cudaSuccess) {
            throw std::runtime_error(std::string("memTypeArray: cudaMemcpy failed: ") +
                                     cudaGetErrorString(err));
        }
    }
