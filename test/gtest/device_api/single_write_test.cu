...
putKernel(putParams put_params,
    ...
    for (size_t i = 0; i < num_iters; ++i) {
        auto status = nixlPut<level>(put_params.src.mvh,
                                     put_params.src.index,
                                     put_params.src.offset,
                                     put_params.dst.mvh,
                                     put_params.dst.index,
                                     put_params.dst.offset,
                                     put_params.size,
                                     put_params.channelId,
                                     put_params.flags,
                                     &xfer_status);
        if (status != NIXL_IN_PROG) {
            printf("Thread %d: nixlPut failed iteration %lu: status=%d (0x%x)\n",
                   threadIdx.x,
                   (unsigned long)i,
                   status,
                   static_cast<unsigned int>(status));
...
TEST_P(SingleWriteTest, SingleWorkerPut) {
    ...
    putParams put_params{{src_mvh}, {dst_mvh}, size};
