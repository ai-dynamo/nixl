...
protected:
    ...
    gtest::scopedTestLogSink logSink_;
};
...
TEST_F(UcxHardwareWarningTest, WarnWhenGpuPresentButCudaNotSupported) {
    ...
    EXPECT_EQ(logSink_.warningCount(), 1);
    EXPECT_EQ(
        logSink_.countWarningsMatching("NVIDIA GPU(s) were detected, but UCX CUDA support was not found"),
        1);
...
TEST_F(UcxHardwareWarningTest, WarnWhenIbPresentButRdmaNotSupported) {
    ...
    EXPECT_EQ(logSink_.warningCount(), 1);
    EXPECT_EQ(logSink_.countWarningsMatching(
...
TEST_F(UcxHardwareWarningTest, NoWarningWhenIbAndCudaSupported) {
    ...
    EXPECT_EQ(logSink_.warningCount(), 0);
