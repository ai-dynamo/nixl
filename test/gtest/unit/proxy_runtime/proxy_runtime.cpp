...
namespace proxy_runtime {
    ...
    waitForCompletedIdx(const nixlProxyChannelView &view, uint64_t completed_idx) {
        ...
        const auto deadline = std::chrono::steady_clock::now() + std::chrono::milliseconds(250);
        while (std::chrono::steady_clock::now() < deadline) {
            if (__atomic_load_n(&completion_slot->completed_idx, __ATOMIC_ACQUIRE) >=
                completed_idx) {
                return true;
            }
            std::this_thread::sleep_for(std::chrono::milliseconds(5));
    ...
    TEST_F(ProxyRuntimeTest, PrepRemoteMemViewStoresResolvedDirectPointers) {
        ...
        backend_->direct_ptrs_to_return_ = {reinterpret_cast<void *>(uintptr_t{0xabc00000}),
                                            nullptr};

        nixlMemViewH dst_proxy = nullptr;
        ASSERT_EQ(
            runtime_.prepMemView(makeRemotePeerDlist({"peer0", "peer1"}, &remote_md), &dst_proxy),
            NIXL_SUCCESS);
