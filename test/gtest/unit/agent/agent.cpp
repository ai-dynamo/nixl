...
namespace agent {
    ...
    TEST_F(dualAgentBridgeFixture, CancelXferReqDelegatesRegardlessOfRequestState) {
        DualAgentSetup s(DRAM_SEG);
        s.local_params["cancel_statuses"] = std::to_string(NIXL_SUCCESS) + "," +
            std::to_string(NIXL_SUCCESS) + "," + std::to_string(NIXL_SUCCESS) + "," +
            std::to_string(NIXL_SUCCESS) + "," + std::to_string(NIXL_ERR_NOT_SUPPORTED) + "," +
            std::to_string(NIXL_IN_PROG);
        setupDualAgent(s);
        ...
        EXPECT_EQ(local_agent_->createXferReq(NIXL_WRITE,
                                              ...
                                              &s.local_extra_params),
                  NIXL_SUCCESS);
        EXPECT_EQ(local_agent_->cancelXferReq(xfer_req), NIXL_SUCCESS);
        EXPECT_EQ(local_agent_->postXferReq(xfer_req), NIXL_IN_PROG);
        EXPECT_EQ(local_agent_->cancelXferReq(xfer_req), NIXL_SUCCESS);
        EXPECT_EQ(local_agent_->cancelXferReq(xfer_req), NIXL_SUCCESS);
        EXPECT_EQ(local_agent_->getXferStatus(xfer_req), NIXL_SUCCESS);
        EXPECT_EQ(local_agent_->cancelXferReq(xfer_req), NIXL_SUCCESS);
        EXPECT_EQ(local_agent_->cancelXferReq(xfer_req), NIXL_ERR_NOT_SUPPORTED);
        EXPECT_EQ(local_agent_->cancelXferReq(xfer_req), NIXL_ERR_BACKEND);
        EXPECT_EQ(local_agent_->releaseXferReq(xfer_req), NIXL_SUCCESS);
    }
