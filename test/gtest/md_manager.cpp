...
// TCPStore-backed manager. Skipped unless NIXL_TCPSTORE_ENDPOINTS (host:port)
// points to a running TCPStore server. NIXL_ETCD_ENDPOINTS is unset so the
// manager selects TCPStore even when ETCD is also configured in the ambient
// environment (ETCD otherwise wins).
...
protected:
    ...
    void
    SetUp() override {
        if (::getenv("NIXL_TCPSTORE_ENDPOINTS") == nullptr) {
        ...
        for (int i = 0; i < AGENT_COUNT_; i++) {
            AgentContext ctx;
            ctx.name = "mdm_tcpstore_agent_" + std::to_string(i) + suffix;
    ...
    // Force TCPStore selection regardless of ambient NIXL_ETCD_ENDPOINTS.
    ScopedUnsetEnv no_etcd_{"NIXL_ETCD_ENDPOINTS"};
    ScopedEnv env_;
    std::vector<AgentContext> agents_;
};

TEST_F(MDManagerTcpStoreFixture, BackendIsTcpStore) {
    EXPECT_EQ(agents_[0].mdm->getBackend(), std::string_view("TCPStore"));
}

TEST_F(MDManagerTcpStoreFixture, RegisterIgnoresAddress) {
    // Centralized backend accepts registration without an address.
    EXPECT_EQ(agents_[0].mdm->registerMDPeer("peer_no_addr", "", 0), NIXL_SUCCESS);
}

TEST_F(MDManagerTcpStoreFixture, SendAndFetchRoundTrip) {
    auto &src = agents_[0];
    auto &dst = agents_[1];
    ...
    ASSERT_EQ(src.mdm->sendLocalMD(dst.name), NIXL_SUCCESS);
    // Fetch is synchronous (check, then get), so the metadata is loaded by the
    // time fetchRemoteMD returns.
    ASSERT_EQ(dst.mdm->fetchRemoteMD(src.name), NIXL_SUCCESS);
    EXPECT_EQ(dst.mdm->checkRemoteMD(src.name, {DRAM_SEG}), NIXL_SUCCESS);
}

TEST_F(MDManagerTcpStoreFixture, PartialRequiresLabel) {
    auto &src = agents_[0];
    auto &dst = agents_[1];
...
TEST_F(MDManagerTcpStoreFixture, InvalidateRemovesPublishedMetadata) {
    auto &src = agents_[0];
    auto &dst = agents_[1];
    ...
    // TCPStore has no watch, so invalidation is observed on the next fetch: the
    // published key is gone, so a fresh fetch reports NOT_FOUND.
    ASSERT_EQ(src.mdm->invalidateLocalMD(dst.name), NIXL_SUCCESS);
    EXPECT_EQ(dst.mdm->fetchRemoteMD(src.name), NIXL_ERR_NOT_FOUND);
}
