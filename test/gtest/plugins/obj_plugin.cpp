...
TEST_P(setupObjTestFixture, readWithOffsetsTest) {
    int seed = getRandomInt(1, 100000);

    // First, write entire buffer to OBJ in a single chunk (no offsets)
    transferMemConfig mem_cfg_write{.numEntries_ = 1, .entrySize_ = 128, .seed_ = seed};
    ...
    transferMemConfig mem_cfg_read{.numEntries_ = 2, .entrySize_ = 64, .seed_ = seed};
