...
// environment variable (default: /dev/loop0).  Tests in this fixture are
// skipped automatically if the device cannot be opened.
// ---------------------------------------------------------------------------
...
protected:
    ...
    void fillPattern(void *buf, size_t size, char pattern)
    {
...
TEST_F(LibblkioIntegrationTest, WriteReadVerify)
{
    ...
    EXPECT_EQ(agent_->getXferStatus(req), NIXL_SUCCESS);
