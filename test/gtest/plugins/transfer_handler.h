...
int
getRandomInt(int min, int max, int seed = 0) {
...
public:
    transferHandler(std::shared_ptr<nixlBackendEngine> src_engine,
        ...
        : srcBackendEngine_(src_engine),
          ...
          srcBufByte_(getRandomInt(0, 255, memConfig_.seed_)),
          dstBufByte_(getRandomInt(0, 255, memConfig_.seed_ * 2)),
    ...
    setSrcMem() {
        for (size_t i = 0; i < srcMem_.size(); i++)
            srcMem_[i]->setIncreasing(srcBufByte_ + i);
    }
    ...
    checkSrcMem() {
        for (size_t i = 0; i < srcMem_.size(); i++)
            EXPECT_TRUE(srcMem_[i]->checkIncreasing(srcBufByte_ + i));
    }
...
private:
    ...
    const uint8_t srcBufByte_;
    const uint8_t dstBufByte_;
