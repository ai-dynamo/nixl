...
> `NIXL_PLUGIN_DIR=/tmp/copied-plugin-dir`, `HF_TOKEN=hf_secret`,
> `https://user:pass@pypi.example/simple`, `python=/home/alice/venv/bin/python`,
> `host=build42.internal.example`, and `ImportError` from
> `/home/alice/venv/lib/python3.12/site-packages/nixl/libplugin_GDS_MT.so`.
...
- Redacts the token, pip index credentials, private hostname, interpreter path,
  ...
  explain the finding.
- Treats `/tmp/copied-plugin-dir` as untrusted or at least not yet trusted, and
