...
`TELEMETRY_VERSION` is the binary layout version of the shared-memory cyclic buffer, covering its header, data placement, and the `nixlTelemetryEvent` entries serialized in it. Bump it whenever the buffer header layout or the struct's size, field offsets, or enum widths change, or when a new event type is added to the stream (readers must know the full event-type set). Readers exact-match this value; the C++ shared-ring-buffer path unlinks the file on mismatch, while the Python example raises a `RuntimeError`. There is no backward-compatibility path. The current version is `5`: it was bumped from `4` when the buffer header was padded to two 256-byte-aligned blocks (512 bytes) to eliminate false sharing between `write_pos` and `read_pos`, which moved `capacity`, `mask`, `read_pos`, and the data-array offset; `version` deliberately stays at byte offset 16 across layout versions so a reader on a mismatched file always reads a real version field. Version `4` was bumped from `3` when the `agent_telemetry_events_dropped` event type (`AGENT_TELEMETRY_EVENTS_DROPPED`) was added.
