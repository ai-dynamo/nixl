...
For the common two-worker setup, use the ASIO runtime. It connects the workers directly over TCP, so no etcd service is required. ASIO supports exactly two workers:
...
Single-instance storage benchmarks also run without etcd.
...
Then run the same command on the peer host. It connects to the listener automatically.
