...
| `NIXL_LOG_FILE` | String (path) | Not set (file logging disabled) | Also writes log records to this file, in the same format and in addition to the existing stderr output. `NIXL_LOG_LEVEL` still selects which records are emitted. The file is appended to rather than truncated, and each record is flushed as it is written so the log survives a crash. If the file cannot be opened, NIXL reports this at error severity, so it is visible even at `NIXL_LOG_LEVEL=ERROR`, and continues without the file; if a later write fails, it reports once on stderr and drops further records rather than holding up the process. Each process needs its own file, so the path may contain `%h` for the host name, `%p` for the process id, `%t` for a per-process run marker in nanoseconds since the Unix epoch, sampled once at startup, and `%%` for a literal `%`. An unrecognized escape is left as written. For example `/var/log/nixl/run_%h_%p_%t.log` can be set once for every worker in a run. Include `%t` if the same command may be run more than once: process ids are recycled, and because the file is appended to, a restart given an earlier run's id would otherwise continue that run's file. The path is expanded once, when the sink is initialized during library load, so a process that calls `fork()` without `exec()` does not get a separate file: its children inherit the parent's already-open file and keep writing to it, under the parent's `%p`. Workers started through `exec`, or by a launcher, each get their own file as expected. |
| `NIXL_LOG_FILE_SIZE` | Size in bytes, optionally suffixed `K`, `M` or `G` | Not set (no limit) | Bounds `NIXL_LOG_FILE`. On reaching the limit the file is renamed with a `.1` suffix, replacing any previous one, and a new file is started, so the most recent records are always the ones kept. Exactly one rotated generation is retained, so the total on disk stays under roughly twice the limit. Without this, the log file grows indefinitely and must be managed externally, for example with `logrotate` using `copytruncate`, or by giving each run a fresh path. Rotation assumes one process per file. If the rotation cannot be done, NIXL reports this on stderr and stops writing to the file, keeping the records written up to that point rather than exceeding the limit. A value that cannot be parsed is reported on stderr and leaves the file unbounded. |
