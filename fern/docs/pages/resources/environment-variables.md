...
| `NIXL_LOG_FILE_SIZE` | At least `4K`, optionally suffixed `K`, `M` or `G` | Not set (no limit) | Sets the rotation size for `NIXL_LOG_FILE`, keeping one rotated generation. See [existing-file behavior](/nixl/user-guide/logging-guide#bounding-the-size). |
