...
execTransferIterations(nixlAgent *agent,
    ...
    nixl_status_t prep_rc = agent->prepXferDlist(NIXL_INIT_AGENT, local_desc, local_dlist_hndl, &params);
    ...
    for (int i = 0; i < num_iter; ++i) {
        nixl_status_t make_rc =
            agent->makeXferReq(op, local_dlist_hndl, local_indices,
                              remote_dlist_hndl, remote_indices, req, &params);
        if (__builtin_expect(make_rc != NIXL_SUCCESS, 0)) {
            std::cerr << "makeXferReq failed: " << nixlEnumStrings::statusStr(make_rc)
                      << std::endl;
            agent->releasedDlistH(local_dlist_hndl);
