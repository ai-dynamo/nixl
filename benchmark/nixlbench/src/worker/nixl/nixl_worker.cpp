...
xferBenchNixlWorker::cleanupBasicDescFile(xferBenchIOV &iov) {
    // Path mode: devId isn't an fd (backend owns it); seed fds closed in deallocateMemory.
    if (xferBenchConfig::storage_enable_path_mode) {
        return;
    }
    close(iov.devId);
    ...
    for (int list_idx = 0; list_idx < num_threads; list_idx++) {
        ...
        for (i = 0; i < num_devices; i++) {
            ...
            if (basic_desc) {
                ...
                // register.
                if (!remote_iovs.empty() && !xferBenchConfig::storage_enable_path_mode) {
...
xferBenchNixlWorker::deallocateMemory(std::vector<std::vector<xferBenchIOV>> &iov_lists) {
    ...
    if (xferBenchConfig::isObjStorageBackend()) {
        ...
        // In path mode cleanupBasicDescFile is a no-op (devId is synthetic, not an
        // fd); the seed fds opened by createFileFds are closed below instead.
