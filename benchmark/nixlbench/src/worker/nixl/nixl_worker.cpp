...
xferBenchNixlWorker::allocateMemory(int num_threads) {
    ...
    if (xferBenchConfig::isObjStorageBackend()) {
        ...
        int expected_num_devices =
            isInitiator() ? xferBenchConfig::num_initiator_dev : xferBenchConfig::num_target_dev;
        libblkio_devices =
            parseLibblkioDeviceList(xferBenchConfig::device_list, expected_num_devices);
        ...
        for (int list_idx = 0; list_idx < num_threads; list_idx++) {
            ...
            for (i = 0; i < num_devices; i++) {
                xferBenchIOV iov(
                    0, buffer_size, libblkio_devices[i].device_id, libblkio_devices[i].device_path);
        ...
        if (XFERBENCH_BACKEND_LIBBLKIO == xferBenchConfig::backend) {
            // LIBBLKIO doesn't use file descriptors - it reads device paths from
            // backend_params["device_list"] Skip file creation and descriptor setup
            remote_fds.clear();
        } else {
            ...
            if (remote_fds.empty()) {
                std::cerr << "Failed to create " << xferBenchConfig::backend << " file"
                          << std::endl;
                exit(EXIT_FAILURE);
            }
        }
