...
DEFINE_string(libblkio_api_type,
              "IO_URING",
              "API type for LIBBLKIO operations [IO_URING, VHOST_USER, VHOST_VDPA] (only used with "
              "LIBBLKIO backend)");
NB_ARG_BOOL(libblkio_io_polling,
            false,
            "Enable I/O polling for LIBBLKIO operations (only used with LIBBLKIO backend)");
    ...
    if (worker_type == XFERBENCH_WORKER_NIXL) {
        ...
        if (backend == XFERBENCH_BACKEND_LIBBLKIO) {
            ...
            if (device_list.empty() || device_list == "all") {
                std::cerr << "LIBBLKIO backend requires explicit device_list, not 'all' or empty"
                          << std::endl;
                return -1;
            }
        }
            ...
            if (xferBenchConfig::isStorageBackend() ||
                ...
                if (xferBenchConfig::op_type == XFERBENCH_OP_READ) {
                    ...
                    if (xferBenchConfig::isObjStorageBackend()) {
                        ...
                        // Map device id -> path via device_list and read from the block device at
                        // LBA offset
                        auto it = std::find_if(libblkio_devs.begin(),
                                               libblkio_devs.end(),
                                               [&](const LibblkioDeviceConfig &m) {
                                                   return m.device_id == iov.devId;
                                               });
