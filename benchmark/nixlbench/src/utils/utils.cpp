...
// Abstraction macros to facilitate transition to boost::program_options.
...
const xferBenchParamInfo xbench_params[] = {
    ...
    NB_ARG_STRING(initiator_seg_type,
                  ...
                  "Type of memory segment for initiator [DRAM, VRAM]. Note: Storage backends "
                  "always use DRAM locally."),
    NB_ARG_STRING(target_seg_type,
                  XFERBENCH_SEG_TYPE_DRAM,
                  "Type of memory segment for target [DRAM, VRAM]. Note: Storage backends "
...
xferBenchConfig::parseConfig(int argc, char *argv[]) {
    ...
    return loadParams(vm);
}

int
xferBenchConfig::loadParams(po::variables_map &vm) {
#define NB_ARG(name) vm[#name].as<decltype(name)>()
