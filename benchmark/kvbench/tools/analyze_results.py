...
def analyze_results(
    ...
    if all_iterations:
        ...
        for iter_idx in range(num_iterations):
            iter_results = analyze_single_iteration(
                data, iter_idx, tp_configs, gpus_per_node
            )
...
def print_numa_analysis(analysis: Dict):
    ...
    for node in nodes:
        ...
        if len(numa0) != len(
            numa1
        ):  # Different number of TPs per NUMA = variation exists
...
def print_outliers(analysis: Dict):
    ...
    for t in tps:
        if t["read_bw_gbs"] > 0:
            ...
            deviation = (
                (t["read_p50_ms"] - expected) / expected * 100 if expected > 0 else 0
            )
            if abs(deviation) > 20:
                outliers.append(
                    {
...
def print_isolated_vs_workload(analysis: Dict):
    ...
    for t in tps:
        if t["read_p50_ms"] > 0 and t["workload_read_ms"] > 0:
            ...
            wkld_bw = (
                (t["read_size_gb"] / (t["workload_read_ms"] / 1000))
                if t["workload_read_ms"] > 0
                else 0
            )
            overhead = (
                ((t["workload_read_ms"] / t["read_p50_ms"]) - 1) * 100
                if t["read_p50_ms"] > 0
                else 0
            )
            data.append(
    ...
    # Per-node breakdown
    node_to_data: dict[int, dict[str, list[float]]] = {}
    for d in data:
        for node in d["nodes"]:
    ...
    if node_to_data and len(node_to_data) > 1:
        ...
        for node in sorted(node_to_data.keys()):
            ...
            ratio = (
                statistics.mean(ndata["wkld_bws"])
                / statistics.mean(ndata["iso_bws"])
                * 100
                if ndata["iso_bws"]
                else 0
            )
...
def print_iteration_variance(analysis: Dict):
    ...
    all_cvs = [
        (
            (t.get("read_bw_gbs_std", 0) / t["read_bw_gbs"] * 100)
            if t["read_bw_gbs"] > 0
            else 0
        )
        for t in tps
...
def print_bandwidth_histogram(analysis: Dict):
    ...
    data = [
        (t["tp"], t["read_bw_gbs"], t["nodes"], t["read_size_gb"])
        for t in tps
        if t["read_bw_gbs"] > 0
    ]
    if not data:
    ...
    for tp, bw, nodes, size_gb in sorted(data, key=lambda x: x[1]):
        ...
        bar = "█" * bar_len
        fast = (
            "🚀" if bw > median_bw * 1.2 else ("🐢" if bw < median_bw * 0.8 else "  ")
        )
...
def print_per_size_analysis(analysis: Dict):
    ...
    for size_gb in sorted(size_groups.keys()):
        ...
        avg_wkld_bw = (
            statistics.mean([w for w in wkld_bws if w > 0])
            if any(w > 0 for w in wkld_bws)
            else 0
        )
...
def print_node_size_matrix(analysis: Dict):
    ...
    nodes = sorted(
        set(t["nodes"][0] for t in tps if t["nodes"] and t["read_bw_gbs"] > 0)
    )
    ...
    for node in nodes:
        ...
        for size_gb in sizes:
            ...
            matching = [
                ...
                if t["nodes"]
                and t["nodes"][0] == node
                and round(t["read_size_gb"], 2) == size_gb
            ]
            if matching:
...
def detect_bottlenecks(analysis: Dict) -> List[Dict]:
    ...
    if tp0 and tp0["read_bw_gbs"] > 0:
        ...
        same_size_tps = [
            t
            for t in tps
            if round(t["read_size_gb"], 2) == size_gb
            and t["tp"] != 0
            and t["read_bw_gbs"] > 0
        ]
        if same_size_tps:
...
def print_aggregate_throughput(analysis: Dict):
    ...
    workload_latencies = [
        t["workload_read_ms"] for t in tps if t["workload_read_ms"] > 0
    ]

    if workload_latencies:
        workload_wall_time_sec = max(workload_latencies) / 1000
        workload_aggregate_bw = (
            total_read_gb / workload_wall_time_sec if workload_wall_time_sec > 0 else 0
        )
    else:
    ...
    if isolated_latencies:
        ...
        isolated_aggregate_bw = (
            total_read_gb / isolated_wall_time_sec if isolated_wall_time_sec > 0 else 0
        )
    else:
    ...
    efficiency = (
        (workload_aggregate_bw / isolated_aggregate_bw * 100)
        if isolated_aggregate_bw > 0
        else 0
    )
...
Examples:
  ...
  %(prog)s results.json --nodelist "hgx-isr1-[098,100,102-103]"
    ...
    parser.add_argument(
        ...
        "-y",
        dest="yaml_path",
        help="Path to traffic pattern YAML (for rank/node mapping)",
    )
    parser.add_argument(
        "--csv", "-c", dest="csv_path", help="Export results to CSV file"
    )
    parser.add_argument(
        "--gpus-per-node", "-g", type=int, default=8, help="GPUs per node (default: 8)"
    )
    parser.add_argument(
        "--iteration",
        "-i",
        type=int,
        default=0,
        help="Which iteration to analyze (default: 0)",
    )
    parser.add_argument(
        "--all-iterations",
        "-a",
        action="store_true",
        help="Analyze all iterations and show variance",
    )
    parser.add_argument(
        "--no-histogram", action="store_true", help="Skip bandwidth histogram"
    )
    parser.add_argument(
        "--no-conclusions",
        action="store_true",
        help="Skip bottleneck detection (show raw data only)",
    )
    parser.add_argument(
        "--nodelist",
        "-n",
        dest="nodelist",
        help='SLURM nodelist (e.g. "hgx-isr1-[098,100]") for physical hostname mapping',
    )
