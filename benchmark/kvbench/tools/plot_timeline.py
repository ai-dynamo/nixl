...
def plot_timeline(
            ...
            tp_timings.append(
                {
                    ...
                    "iso_p50_ms": tp.get(
                        "isolated_read_p50_ms", tp.get("isolated_rdma_p50_ms", 0)
                    ),
                }
            )
    ...
    ax1.set_title(
        f"TP Execution Timeline (Iteration {iteration})", fontsize=14, fontweight="bold"
    )
    ...
    for timing in tp_timings:
        ...
        if timing["duration_ms"] > 50:  # Only show for visible bars
            ax1.text(
                timing["start"] + (timing["end"] - timing["start"]) / 2,
                tp_idx,
    ...
    legend_handles = [
        mpatches.Patch(color=node_colors[i], label=f"Node {i}")
    ...
    ax2.axhline(
        y=1, color="red", linestyle="--", alpha=0.5, label="Sequential (no overlap)"
    )
    ...
    if tp_nodes:
        ...
        for timing in tp_timings:
            ...
            node_times[node]["first_start"] = min(
                node_times[node]["first_start"], timing["start"]
            )
            node_times[node]["last_end"] = max(
                node_times[node]["last_end"], timing["end"]
            )
...
def main():
    ...
    parser.add_argument(
        "--iteration", "-i", type=int, default=0, help="Iteration to plot"
    )
    parser.add_argument(
        "--order", action="store_true", help="Plot TP execution order comparison"
    )
    parser.add_argument(
        "--text",
        "-t",
        action="store_true",
        help="Print ASCII Gantt chart (no matplotlib)",
    )
