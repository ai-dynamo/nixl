...
nixlTcpStoreMetadataBackend::fetchRemote(const std::string &remote_name,
    ...
    try {
        ...
        if (loaded_name != remote_name) {
            ...
            NIXL_ERROR << "[" << ctx_.getName() << "] TCPStore metadata for " << remote_name
...
nixlTcpStoreMetadataBackend::invalidateLocal(const nixl_opt_args_t * /*extra_params*/) {
    ...
    // serializes with publishKey (see there); lock order is always
    // publishedMutex_ -> client mutex, so there is no deadlock with fetchRemote.
    const std::lock_guard<std::mutex> lk(publishedMutex_);
    try {
    ...
    NIXL_DEBUG << "[" << ctx_.getName() << "] TCPStore invalidated " << count << " key(s)";
    return NIXL_SUCCESS;
}
