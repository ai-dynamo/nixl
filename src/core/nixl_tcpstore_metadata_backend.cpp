...
constexpr char kNamespace[] = "/nixl/agents";
...
makeKey(const std::string &label, const std::string &src, const std::string &dst) {
    return std::string(kNamespace) + "/" + label + "/" + src + "/" + dst;
}
...
nixlTcpStoreMetadataBackend::nixlTcpStoreMetadataBackend(nixlMetadataContext &ctx) : ctx_(ctx) {
    ...
    const long timeout_ms =
        nixl::config::getValueDefaulted<long>("NIXL_TCPSTORE_TIMEOUT_MS", kDefaultTimeoutMs);
...
nixlTcpStoreMetadataBackend::sendLocal(const nixl_opt_args_t * /*extra_params*/) {
    ...
    return publishKey(makeKey(default_metadata_label, ctx_.getName(), nixl_null_agent), blob);
}
...
nixlTcpStoreMetadataBackend::sendLocalPartial(const nixl_reg_dlist_t &descs,
    ...
    return publishKey(makeKey(extra_params->metadataLabel, ctx_.getName(), nixl_null_agent), blob);
}
...
nixlTcpStoreMetadataBackend::fetchRemote(const std::string &remote_name,
    ...
    try {
        if (!client_->check(key)) {
            NIXL_DEBUG << "[" << ctx_.getName() << "] TCPStore key not yet present: " << key;
        ...
        if (ret < 0) {
            NIXL_ERROR << "[" << ctx_.getName() << "] failed to load metadata fetched for "
                       << remote_name << " with status " << ret;
            return ret;
        }
        NIXL_DEBUG << "[" << ctx_.getName() << "] TCPStore fetched metadata for " << remote_name;
        return NIXL_SUCCESS;
    }
    catch (const std::exception &e) {
...
nixlTcpStoreMetadataBackend::invalidateLocal(const nixl_opt_args_t * /*extra_params*/) {
    ...
    {
        const std::lock_guard<std::mutex> lk(publishedMutex_);
        to_delete.assign(publishedKeys_.begin(), publishedKeys_.end());
    }
    ...
    try {
        ...
        NIXL_ERROR << "[" << ctx_.getName() << "] TCPStore invalidate failed: " << e.what();
        return NIXL_ERR_BACKEND;
    }

    {
        const std::lock_guard<std::mutex> lk(publishedMutex_);
        for (const auto &key : to_delete) {
            publishedKeys_.erase(key);
        }
    }
    NIXL_DEBUG << "[" << ctx_.getName() << "] TCPStore invalidated " << to_delete.size()
               << " key(s)";
    return NIXL_SUCCESS;
}
