        ...
        if (!fini_func_name.empty()) {
            ...
            if (fini) {
                try {
                    ...
                    NIXL_ERROR << "Exception in plugin cleanup (" << fini_func_name
                               << "): " << e.what();
                }
                catch (...) {
                    NIXL_ERROR << "Unknown exception in plugin cleanup (" << fini_func_name << ")";
                }
            }
        }
...
basePluginManager::loadPluginFromPath(const std::filesystem::path &plugin_path) {
...
basePluginManager::loadPluginInternal(const std::string &plugin_name) {
    ...
    for (const auto &dir : pluginDirs_) {
        ...
        // Load the plugin
        auto result = loadPluginFromPath(plugin_path);
