...
void nixlAgent::updateRequestStats(const nixlXferReqH* req_hndl) const {
    if (!nixlTelemetry::getInstance()->isEnabled()) {
        return;
    }
    if (req_hndl->status == NIXL_SUCCESS) {
        auto duration = std::chrono::high_resolution_clock::now() - req_hndl->transaction_start_time;
            nixlTelemetry::getInstance()->updateTransactionTime(
                std::chrono::duration_cast<std::chrono::microseconds>(
                   duration));
        if (req_hndl->backendOp == NIXL_WRITE) {
            nixlTelemetry::getInstance()->updateTxBytes(req_hndl->totalBytes);
            nixlTelemetry::getInstance()->updateTxRequestsNum(1);
        } else {
            nixlTelemetry::getInstance()->updateRxBytes(req_hndl->totalBytes);
            nixlTelemetry::getInstance()->updateRxRequestsNum(1);
        }
    } else {
        // don't count success
        nixlTelemetry::getInstance()->updateErrorCount(req_hndl->status);
    }
}
