...
inline void
setNotification(const nixl_opt_args_t *extra_params, nixl_opt_b_args_t &opt_args) {
    if (__builtin_expect(extra_params == nullptr, 0)) {
        return;
    }

    if (__builtin_expect(static_cast<bool>(extra_params->notif), 1)) {
        opt_args.notifMsg = *extra_params->notif;
        opt_args.hasNotif = true;
    } else if (__builtin_expect(extra_params->hasNotif, 0)) {
...
nixlAgent::makeXferReq (const nixl_xfer_op_t &operation,
    ...
    setNotification(extra_params, opt_args);
...
nixlAgent::postXferReq(nixlXferReqH *req_hndl,
    ...
    if (extra_params) {
        ...
        if (opt_args.hasNotif) {
            req_hndl->notifMsg = opt_args.notifMsg;
        }
    }
