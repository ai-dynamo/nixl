...
class nixl_xfer_handle:
    ...
    def cancel(self):
        ...
        Supporting backends accept repeated calls regardless of request state. Continue polling an
        active request to a terminal status before reusing its buffers or releasing or dropping the
        handle. Backends without cancellation support raise an error.
        """
        if not self._released:
...
class nixl_agent:
    ...
    def cancel_xfer(self, handle: nixl_xfer_handle):
        ...
        Supporting backends accept repeated calls regardless of request state. For an active request,
        continue calling check_xfer_state until completion before reusing its buffers or releasing
        the handle. Backends without cancellation support raise an error.
