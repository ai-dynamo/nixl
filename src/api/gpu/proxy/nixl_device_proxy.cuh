...
nixlProxyChannelIndex(const nixlProxyDeviceContextData &context,
                      uint32_t peer_index,
                      uint32_t channel_id) {
    return static_cast<size_t>(channel_id) * context.peer_capacity + peer_index;
}
...
nixlProxyEnqueue(const nixlProxyDeviceContextData &context,
    ...
    if (submission.dst_index >= context.peer_capacity || context.num_channels == 0 ||
...
// Poll the completion slot recorded by nixlProxyEnqueue().
__device__ inline nixl_status_t
