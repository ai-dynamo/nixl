...
fn test_get_xfer_telemetry_success() {
...

    {
        ...
        let telemetry_result = agent1.get_xfer_telemetry(&xfer_req);
...
fn test_get_xfer_telemetry_without_telemetry_enabled() {
...

    {
        ...
        // Try to get telemetry data - should fail with TelemetryNotEnabled
        let telemetry_result = agent1.get_xfer_telemetry(&xfer_req);
        ...
        match telemetry_result.err().unwrap() {
            NixlError::TelemetryNotEnabled => {
                println!("Correctly received TelemetryNotEnabled error");
            }
            other => panic!("Expected TelemetryNotEnabled error, got: {:?}", other),
        }
    }
}
...
fn test_get_xfer_telemetry_before_posting() {
...

    {
        ...
        match error {
            NixlError::TelemetryNotEnabled | NixlError::BackendError => {
                println!("Got expected error before posting: {:?}", error);
            }
            other => panic!("Expected TelemetryNotEnabled or BackendError, got: {:?}", other),
        }
