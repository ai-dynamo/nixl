...
class nixlPosixAioIOQueue : public nixlPosixIOQueue {
public:
    nixlPosixAioIOQueue() {}
...
private:
    uint32_t num_ios_;
    std::vector<nixlPosixAioIO> ios_; // Array of ios
    std::list<nixlPosixAioIO *> free_ios_; // List of free ios
    std::list<nixlPosixAioIO *> ios_to_submit_; // List of ios to submit
    std::list<nixlPosixAioIO *> ios_in_flight_; // List of ios that have been submitted
};
...
NIXL_POSIX_IO_QUEUE_REGISTER("POSIXAIO", nixlPosixAioIOQueue);
