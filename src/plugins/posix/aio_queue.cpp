...
aioQueue::~aioQueue() {
    ...
    if (num_ios_outstanding > 0) {
        NIXL_ERROR << "Programming error: Destroying aioQueue with " << num_ios_outstanding << " in-flight I/Os";
    }
    ...
    for (int i = start_idx; i < start_idx + count; i++) {
        ...
        if (ret < 0) {
            if (errno == EAGAIN) {
                ...
                NIXL_ERROR << absl::StrFormat("AIO submit partial due to EAGAIN: %d/%d", submitted_count, count);
...
nixl_status_t aioQueue::checkCompleted() {
    ...
    for (int i = 0; i < num_ios_submitted_total; i++) {
        if (completed[i])
            continue; // Skip already completed I/Os
