...
class nixlPosixLinuxAioIOQueue : public nixlPosixIOQueue {
public:
    nixlPosixLinuxAioIOQueue() : initialized_(false) {}
...
private:
    ...
    std::vector<nixlPosixLinuxAioIO> ios_; // Array of ios
    std::list<nixlPosixLinuxAioIO *> free_ios_; // List of free ios
    std::list<nixlPosixLinuxAioIO *> ios_to_submit_; // List of ios to submit
};

nixl_status_t
nixlPosixLinuxAioIOQueue::init(uint32_t max_ios) {
    num_ios_ = normalizedMaxIOS(max_ios);

    int res = io_queue_init(num_ios_, &io_ctx_);
    if (res) {
        NIXL_ERROR << "io_queue_init failed: " << res;
...
NIXL_POSIX_IO_QUEUE_REGISTER("AIO", nixlPosixLinuxAioIOQueue);
