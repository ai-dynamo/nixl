...
class nixlPosixIoUringIOQueue : public nixlPosixIOQueue {
public:
    nixlPosixIoUringIOQueue() : initialized_(false) {}

    virtual nixl_status_t
    init(uint32_t max_chunks) override;
    virtual nixl_status_t
    post(void) override;
    virtual nixl_status_t
    enqueue(int fd,
...
private:
    ...
    std::vector<nixlPosixIoUringIO> ios_; // Array of ios
    std::list<nixlPosixIoUringIO *> free_ios_; // List of free ios
    std::list<nixlPosixIoUringIO *> ios_to_submit_; // List of ios to submit
};

nixl_status_t
nixlPosixIoUringIOQueue::init(uint32_t max_ios) {
    num_ios_ = normalizedMaxIOS(max_ios);

    io_uring_params params = {};
    if (io_uring_queue_init_params(num_ios_, &uring, &params) < 0) {
        NIXL_ERROR << absl::StrFormat("Failed to initialize io_uring instance: %s",
                                      nixl_strerror(errno));
        return NIXL_ERR_BACKEND;
    }

    ios_.reserve(num_ios_);
    for (uint32_t i = 0; i < num_ios_; i++) {
        ios_[i].idx_ = i;
...
nixlPosixIoUringIOQueue::doCheckCompleted(void) {
    ...
    io_uring_for_each_cqe(&uring, head, cqe) {
        ...
        nixlPosixIoUringIO *io =
            reinterpret_cast<nixlPosixIoUringIO *>(io_uring_cqe_get_data(cqe));
        if (io->clb_) {
...
NIXL_POSIX_IO_QUEUE_REGISTER("URING", nixlPosixIoUringIOQueue);
