...
class nixlNullBackendReqH : public nixlBackendReqH {
public:
    nixlNullBackendReqH() = default;
    ~nixlNullBackendReqH() = default;
};

class nixlNullEngine : public nixlBackendEngine {
private:
    mutable nixlNullBackendReqH null_request_;  // Reusable request handle for minimal overhead

public:
    nixlNullEngine(const nixlBackendInitParams* init_params);
    virtual ~nixlNullEngine() = default;
    ...
    nixl_mem_list_t getSupportedMems() const override {
        // Support all memory types for testing
