...
- **Gauge**: Shows the value per the last event (transaction) and can grow or decrease as each event updates it. The byte gauges are exposed with a `_last` suffix to keep them distinct from the cumulative `_total` counter of the same base name: `agent_tx_bytes_last` / `agent_rx_bytes_last` carry the byte size of the latest TX/RX request, while `agent_tx_bytes_total` / `agent_rx_bytes_total` carry the running total. The memory gauges keep their non-suffixed names (`agent_memory_registered` / `agent_memory_deregistered`) and report the size of the last (de)registration.
