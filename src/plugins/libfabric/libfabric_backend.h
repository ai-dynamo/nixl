...
public:
    ...
    void init_request_tracking(size_t num_requests);

    /** Atomically increment completed request count */
    void increment_completed_requests();

    /** Get current count of completed requests */
    size_t get_completed_requests_count() const;

    /** Get total number of requests used for this transfer */
    size_t get_total_requests_used() const;

    /** Get reference to binary notification for XFER_ID collection */
    BinaryNotification& getBinaryNotification() {
...
class nixlLibfabricEngine
    ...
    private:
        ...
        struct PendingNotification {
            ...
            PendingNotification(const std::string& agent, const std::string& msg,
                              const std::unordered_set<uint32_t>& xfer_ids)
                : remote_agent(agent), message(msg), expected_xfer_ids(xfer_ids),
                  received_time(std::chrono::steady_clock::now()) {}
        };
        std::vector<PendingNotification> pending_notifications_;

        // Connection management helpers
        nixl_status_t establishConnection(const std::string& remote_agent) const;

        // Common connection creation helper
        nixl_status_t createAgentConnection(
        ...
        nixl_status_t cmThread();
        void postShutdownCompletion();
        // Progress thread for data rail CQs only
        nixl_status_t progressThread();


    // Engine message processing methods
    void processNotification(const std::string& serialized_notif);
    void processConnectionAck(uint16_t agent_idx, nixlLibfabricConnection* conn_info, ConnectionState state);
    nixl_status_t processConnectionRequest(uint16_t agent_idx, const std::string& serialized_data, nixlLibfabricRail* rail);
    ...
    public:
         ...
         * 2. Sends disconnect notification message to the remote peer via control rail, with best effort semantics
         ...
         * @warning This operation is irreversible; a new connection must be established to resume communication
         *
