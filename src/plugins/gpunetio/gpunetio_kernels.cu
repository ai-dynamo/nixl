...
nixl_gpunetio_dev_priv_poll_one_cq_at(struct doca_gpu_dev_verbs_cq *cq, uint64_t cons_index) {
...
template<enum doca_gpu_dev_verbs_resource_sharing_mode resource_sharing_mode =
         ...
         enum doca_gpu_dev_verbs_qp_type qp_type = DOCA_GPUNETIO_VERBS_QP_SQ>
__device__ static inline int
nixl_gpunetio_dev_poll_one_cq_at(struct doca_gpu_dev_verbs_cq *cq, uint64_t cons_index) {
    int status =
