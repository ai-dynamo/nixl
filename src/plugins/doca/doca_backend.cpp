...
nixl_status_t nixlDocaEngine::nixlDocaInitNotif(const std::string &remote_agent,
	...
	result = create_doca_mmap(notif->send_addr, notif->elems_num, notif->elems_size, ddev, &notif->send_mmap);
	if (result != DOCA_SUCCESS)
		goto error;

	result = create_doca_buf_arr(notif->send_mmap, notif->elems_num, (size_t)notif->elems_size, gpu,
								&(notif->send_barr), &(notif->send_barr_gpu));
	if (result != DOCA_SUCCESS)
		goto error;
	...
	result = create_doca_mmap(notif->recv_addr, notif->elems_num, notif->elems_size, ddev, &notif->recv_mmap);
	if (result != DOCA_SUCCESS)
		goto error;

	result = create_doca_buf_arr(notif->recv_mmap, notif->elems_num, (size_t)notif->elems_size, gpu,
                                    &(notif->recv_barr), &(notif->recv_barr_gpu));
	if (result != DOCA_SUCCESS)
		goto error;
...
nixl_status_t nixlDocaEngine::nixlDocaDestroyNotif(struct doca_gpu *gpu,
									 struct nixlDocaNotif *notif) {
...
nixl_status_t nixlDocaEngine::progressThreadStart() {
	...
	if (bind(oob_sock_server, (struct sockaddr *)&server_addr,
		...
		NIXL_ERROR << "Couldn't bind to the port";
		close(oob_sock_server);
		return NIXL_ERR_NOT_SUPPORTED;
	}
	...
	result = pthread_create(&server_thread_id, NULL, threadProgressFunc, (void *)this);
	if (result != 0) {
...
nixl_status_t nixlDocaEngine::addRdmaQp(const std::string &remote_agent) {
	...
	NIXL_DEBUG << "DOCA addRdmaQp for remote " << remote_agent;
	...
	if (rdma_qp->rdma_ctx_data == NULL) {
		...
		NIXL_ERROR << "Failed to convert DOCA RDMA to DOCA context " << doca_error_get_descr(result);
		return NIXL_ERR_BACKEND;
	}
	...
	// 	if (result != DOCA_SUCCESS) {
	// 		NIXL_ERROR << "Failed to set gid_index to DOCA RDMA " << // doca_error_get_descr(result); return NIXL_ERR_BACKEND;
	// 	}
	// }
	...
	if (result != DOCA_SUCCESS) {
		NIXL_ERROR << "Failed to set send queue size to DOCA RDMA " << doca_error_get_descr(result);
		return NIXL_ERR_BACKEND;
	}
	...
	if (result != DOCA_SUCCESS) {
		NIXL_ERROR << "Failed to set receive queue size to DOCA RDMA " << doca_error_get_descr(result);
		return NIXL_ERR_BACKEND;
	}
...
nixl_status_t nixlDocaEngine::connectClientRdmaQp(int oob_sock_client,
	...
	NIXL_ERROR << "Receive remote data qp connection details";
	...
	if (remote_conn_details_len_data <= 0 ||
		...
		NIXL_ERROR << "Received wrong remote connection details";
...
nixl_status_t nixlDocaEngine::connectServerRdmaQp(int oob_sock_client,
									const std::string &remote_agent) {
	doca_error_t result;
	void *remote_conn_details_data = NULL;
	...
	if (remote_conn_details_notif == NULL) {
		NIXL_ERROR << "Failed to allocate memory for remote notif details " << remote_conn_details_notif_len;
		result = DOCA_ERROR_NO_MEMORY;
		return NIXL_ERR_BACKEND;
	}
...
nixl_status_t nixlDocaEngine::loadRemoteConnInfo(const std::string &remote_agent,
	...
	if (addRdmaQp(remote_agent) == NIXL_SUCCESS) {
		...
		NIXL_WARN << "loadRemoteConnInfo: agent " << remote_agent << " already present";
										  ...
										  nixlBackendMD *&out)
{
	...
	/* Local buffer array */
	result = create_doca_buf_arr(priv->mem.mmap, 1, (size_t)mem.len, gdevs[0].second,
								&(priv->mem.barr), &(priv->mem.barr_gpu));
	if (result != DOCA_SUCCESS)
		goto error;
...
nixl_status_t nixlDocaEngine::loadRemoteMD(const nixlBlobDesc &input,
	...
	if (result != DOCA_SUCCESS) {
		NIXL_ERROR << "Function doca_mmap_create_from_export failed " << doca_error_get_descr(result);
		return NIXL_ERR_BACKEND;
	}

	/* Remote buffer array */
	result = create_doca_buf_arr(md->mem.mmap, 1, (size_t)size, gdevs[0].second,
                                    &(md->mem.barr), &(md->mem.barr_gpu));
	if (result != DOCA_SUCCESS)
		goto error;
	...
	nixlBackendReqH *&handle, const nixl_opt_b_args_t *opt_args) const
{
	...
	if (opt_args && opt_args->hasNotif) {
		...
		NIXL_DEBUG << "DOCA prepXfer with notif to " << remote_agent
				   << " at " << xferReqRingCpu[treq->end_pos - 1].has_notif_msg_idx
				   << " msg " << newMsg
				   << " to " << remote_agent;
...
nixl_status_t nixlDocaEngine::genNotif(const std::string &remote_agent,
	...
	if (msg.size() > DOCA_MAX_NOTIF_MESSAGE_SIZE - msg_tag_start.size() - msg_tag_end.size() - 16) {
		NIXL_ERROR  << "Can't send notif as message size " << msg.size()
	...
	NIXL_DEBUG 	<< "genNotif to " << remote_agent
				<< " msg size " << std::to_string((int)msg.size())
				<< " msg " << newMsg
				<< " at " << buf_idx;
