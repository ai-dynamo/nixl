...
#ifndef LIBBLKIO_BACKEND_H
#define LIBBLKIO_BACKEND_H
...
public:
    ...
    nixlLibblkioBackendMD(nixl_mem_t type, uint64_t addr, size_t len, uint64_t devId)
        : nixlBackendMD(true), memType(type), addr(addr), len(len), devId(devId) {}
...
public:
    ...
    ~nixlLibblkioBackendReqH() = default;
     ...
     */
    nixl_status_t register_blkio_buf(struct blkio *handle, void *buf, size_t size);

    /** Unmap all previously registered DRAM buffers. */
    void unregister_blkio_bufs();
...
private:
    ...
    struct blkio *getBlkioHandle(uint64_t devId) const;
     ...
     * @return NIXL_SUCCESS on success, error code on failure.
     */
    nixl_status_t createBlkioDevice(const std::string &path, uint64_t devId);
};

#endif // LIBBLKIO_BACKEND_H
