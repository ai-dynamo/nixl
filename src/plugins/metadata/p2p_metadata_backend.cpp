...
connectToIP(const std::string &ip_addr, std::uint16_t port) {
    ...
    nixl::metadata::socket_fd ret_fd(socket(AF_INET, SOCK_STREAM | SOCK_NONBLOCK, 0));
    if (!ret_fd) {
        NIXL_ERROR << "socket creation failed for ip_addr: " << ip_addr << " and port: " << port;
        return {};
    }

    int ret = connect(ret_fd.get(), reinterpret_cast<sockaddr *>(&listenerAddr), sizeof(listenerAddr));
...
sendCommMessage(int fd, const std::string &msg) {
    ...
    iovec iov[iov_size] = {{&size, sizeof(size)}, {const_cast<char *>(msg.data()), msg.size()}};
...
bool
parsePeerKey(const std::string &key, std::string &ip, std::uint16_t &port) {
    ...
    try {
        const std::string port_str = key.substr(colon + 1);
        size_t parsed = 0;
        const unsigned long parsed_port = std::stoul(port_str, &parsed);
...
socket_fd::socket_fd(int fd) noexcept
    : fd_(fd) {}
...
socket_fd::socket_fd(socket_fd &&other) noexcept
...
int
socket_fd::get() const noexcept {
    return fd_;
}

socket_fd::operator bool() const noexcept {
    return fd_ >= 0;
}

nixlP2PMetadataBackend::nixlP2PMetadataBackend(std::uint16_t listen_port,
...
nixl_status_t
nixlP2PMetadataBackend::ensureSocket(const std::string &ip, std::uint16_t port) {
    if (socketFor(ip, port) != -1) {
        return NIXL_SUCCESS;
    }
    auto fd = connectToIP(ip, port);
    if (!fd) {
        NIXL_ERROR << "Listener thread could not connect to IP " << ip << " and port " << port;
        return NIXL_ERR_BACKEND;
    }
    sockets_.emplace(socket_peer_t{ip, port}, std::move(fd));
    return NIXL_SUCCESS;
}

int
nixlP2PMetadataBackend::socketFor(const std::string &ip, std::uint16_t port) const {
...
void
nixlP2PMetadataBackend::forgetSocket(const std::string &ip, std::uint16_t port) {
    const auto it = sockets_.find({ip, port});
    if (it == sockets_.end()) {
        return;
    }
...
void
nixlP2PMetadataBackend::forgetSocket(socket_map_t::iterator it) {
    if (it == sockets_.end()) {
        return;
    }
    sockets_.erase(it);
...
nixl_status_t
nixlP2PMetadataBackend::requestMetadataFromPeer(const std::string &ip, std::uint16_t port) {
    const nixl_status_t ret = ensureSocket(ip, port);
    if (ret != NIXL_SUCCESS) {
        return ret;
    }
...
nixlP2PMetadataBackend::processOnce(nixlAgent &agent) {
    ...
    for (;;) {
        socket_fd new_fd(listener_->acceptClient());
        if (!new_fd) {
            break;
        }
        const int fd = new_fd.get();

        sockaddr_in client_address{};
        socklen_t client_addrlen = sizeof(client_address);
        if (getpeername(fd, (sockaddr *)&client_address, &client_addrlen) != 0) {
        ...
        const int cur_flags = fcntl(fd, F_GETFL, 0);
        if (cur_flags == -1 || fcntl(fd, F_SETFL, cur_flags | O_NONBLOCK) == -1) {
            NIXL_PERROR << "fcntl on accepted client failed";
            continue;
        }

        sockets_.emplace(socket_peer_t{std::string(client_ip), ntohs(client_address.sin_port)},
                         std::move(new_fd));
    }
    ...
    while (it != sockets_.end()) {
        ...
        try {
            const bool received = recvCommMessage(it->second.get(), commands);
            if (!received) {
                ++it;
                continue;
            }
        ...
        if (commands.rfind(prefix, 0) != 0 || commands.size() < prefix.size() + 4) {
            ...
            if (header == "LOAD") {
                ...
                if (ret != NIXL_SUCCESS) {
                    ...
                    try {
                        sendCommMessage(it->second.get(), std::string("NIXLCOMM:LOAD" + my_md));
                    }
                    catch (const std::runtime_error &e) {
                        NIXL_ERROR << "Failed to send message to peer, disconnecting: " << e.what();
                        disconnected = true;
