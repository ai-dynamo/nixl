...
We've created [`run_nixlbench_tcpxo.sh`](/src/plugins/gpudirect_tcpxo/scripts/run_nixlbench_tcpxo.sh)
(which uses [`nixl_test_utils.sh`](/src/plugins/gpudirect_tcpxo/scripts/nixl_test_utils.sh)) to simplify
...
1.  **IAM Permissions**
    ...
    *   Access to pull RxDM images from
        [gce-ai-infra](https://us-docker.pkg.dev/gce-ai-infra/gpudirect-tcpxo).
...
different run modes. Run `./run_nixlbench_tcpxo.sh -h` to see all the options
you can tweak.
...
*   **Barrier Synchronization Timeout / Hanging:** If the test execution hangs
    ...
    have completed registration with the ETCD server.
