...
nixlTcpxoEngine::registerMem(const nixlBlobDesc &mem,
    ...
    if (auto it = mem_device.cache.find(region_info); it != mem_device.cache.end()) {
        ...
        auto [inserted_itr, inserted] = mem_device.cache.insert(
            {region_info,
             CacheValueType{
                 std::make_unique<nixlTcpxoLocalMemoryMetadata>(
                     reg_handle, reinterpret_cast<void *>(addr), size, dmabuf_fd, fastrak_idx),
                 1}});
        if (!inserted) {
            NIXL_ERROR << absl::StrFormat(
...
nixlTcpxoEngine::ValidateXferDlists(const nixl_xfer_op_t &operation,
    ...
    for (int idx = 0; idx < local.descCount(); ++idx) {
        ...
        if (!fastrak_idx_or_status.ok()) {
            return absl::InvalidArgumentError(fastrak_idx_or_status.status().message());
        }
        const uint8_t fastrak_idx = *fastrak_idx_or_status;
