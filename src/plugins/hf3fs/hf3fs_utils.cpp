...
nixl_status_t hf3fsUtil::registerFileHandle(int fd, int *ret)
{
	...
	if (ret_val > 0) {
        HF3FS_LOG_RETURN(NIXL_ERR_BACKEND,
            absl::StrFormat("Error registering file descriptor %d, error: %d",
                            fd, ret_val));
	}
	*ret = ret_val;
	return NIXL_SUCCESS;
...
nixl_status_t hf3fsUtil::wrapIOV(struct hf3fs_iov *iov, void *addr, size_t size, size_t block_size)
{
    ...
    if (ret < 0) {
        HF3FS_LOG_RETURN(NIXL_ERR_BACKEND,
            absl::StrFormat("Error wrapping memory into IOV, error: %d", ret));
    }
...
nixl_status_t hf3fsUtil::createIOR(struct hf3fs_ior *ior, int num_ios, bool is_read)
{
    ...
    if (ret < 0) {
        HF3FS_LOG_RETURN(NIXL_ERR_BACKEND,
            absl::StrFormat("Error creating IOR, error: %d", ret));
    }
                                   ...
                                   size_t block_size)
{
    ...
    if (ret < 0) {
        HF3FS_LOG_RETURN(NIXL_ERR_BACKEND,
            absl::StrFormat("Error creating IOV, error: %d", ret));
    }
                                ...
                                void *user_data)
{
    ...
    if (ret < 0) {
        HF3FS_LOG_RETURN(NIXL_ERR_BACKEND,
            absl::StrFormat("Error: hf3fs prep io error: %d", ret));
    }
...
nixl_status_t hf3fsUtil::postIOR(struct hf3fs_ior *ior)
{
    ...
    if (ret < 0) {
        HF3FS_LOG_RETURN(NIXL_ERR_BACKEND,
            absl::StrFormat("hf3fs submit ios error: %d", ret));
    }
                                    ...
                                    int min_cqes, struct timespec *ts, int *num_completed)
{
    ...
    if (ret < 0 && ret != -ETIMEDOUT && ret != -EAGAIN) {
        HF3FS_LOG_RETURN(NIXL_ERR_BACKEND,
            absl::StrFormat("Error waiting for IOs: %d", ret));
    }
