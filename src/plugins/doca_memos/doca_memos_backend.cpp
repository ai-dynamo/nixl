...
void
nixlDocaMemosEngine::NvmeKvdevDeleter::operator()(doca_kv_nvme_kernel_dev *dev) const noexcept {
    if (!dev) {
        return;
    }
    // A device is live from create to destroy; there is no started state to unwind.
    doca_error_t result = doca_kv_nvme_kernel_dev_destroy(dev);
...
void
nixlDocaMemosEngine::NvmeKvdevAttrDeleter::operator()(
    doca_kv_nvme_kernel_dev_create_attr *attr) const noexcept {
    if (!attr) {
        return;
    }
    doca_error_t result = doca_kv_nvme_kernel_dev_create_attr_destroy(attr);
    if (result != DOCA_SUCCESS) {
        NIXL_WARN << "Failed to destroy DOCA KV device attributes: "
...
nixlDocaMemosEngine::parseInitParams(const nixl_b_params_t *params) {
    ...
    // The task-pool ceiling belongs to the I/O context, not the device, so it can be
    // applied here rather than after the device exists. doca_kv_io_set_num_tasks()
    // rejects anything above it outright.
    uint32_t io_max_tasks = 0;
    doca_error_t result = doca_kv_io_cap_get_max_tasks(&io_max_tasks);
    if (result != DOCA_SUCCESS) {
        NIXL_ERROR << "doca_kv_io_cap_get_max_tasks failed: " << doca_error_get_descr(result);
        return NIXL_ERR_BACKEND;
    }
    if (io_max_tasks > 0 && numTasks_ > io_max_tasks) {
        NIXL_INFO << "Clamping num_tasks from " << numTasks_ << " to I/O context max "
                  << io_max_tasks;
        numTasks_ = io_max_tasks;
    }
...
nixlDocaMemosEngine::initDocaDevice() {
    ...
    doca_error_t result = doca_kv_nvme_kernel_dev_cap_get_max_path_len(&max_path_len);
    if (result != DOCA_SUCCESS) {
        NIXL_ERROR << "doca_kv_nvme_kernel_dev_cap_get_max_path_len failed: "
                   << doca_error_get_descr(result);
        return NIXL_ERR_BACKEND;
    }
    ...
    uint8_t nguid_bytes[DOCA_KV_DEV_NGUID_LEN] = {0};
    if (!nguid_.empty()) {
        if (nguid_.length() != DOCA_KV_DEV_NGUID_LEN * 2) {
            NIXL_ERROR << "Invalid nguid '" << nguid_ << "' (expected 32 hex chars)";
            return NIXL_ERR_INVALID_PARAM;
        }
        for (size_t i = 0; i < DOCA_KV_DEV_NGUID_LEN; i++) {
            unsigned val = 0;
    ...
    // The device is configured through an attribute object and validated at create:
    // the path and the NGUID are both checked against the underlying namespace there,
    // so a mismatch surfaces as a create failure rather than a later start failure.
    doca_kv_nvme_kernel_dev_create_attr *raw_attr = nullptr;
    result = doca_kv_nvme_kernel_dev_create_attr_create(&raw_attr);
    if (result != DOCA_SUCCESS) {
        NIXL_ERROR << "Failed to create DOCA KV device attributes: "
                   << doca_error_get_descr(result);
        return NIXL_ERR_BACKEND;
    }
    std::unique_ptr<doca_kv_nvme_kernel_dev_create_attr, NvmeKvdevAttrDeleter> attr(raw_attr);

    result = doca_kv_nvme_kernel_dev_create_attr_set_path(attr.get(), deviceName_.c_str());
    if (result != DOCA_SUCCESS) {
        NIXL_ERROR << "Failed to set NVMe kernel KV device path: " << doca_error_get_descr(result);
        return NIXL_ERR_BACKEND;
    }

    doca_kv_dev_create_attr *kv_attr =
        doca_kv_nvme_kernel_dev_create_attr_as_kv_dev_create_attr(attr.get());
    if (!kv_attr) {
        NIXL_ERROR << "Failed to convert NVMe kernel KV device attributes to generic";
        return NIXL_ERR_BACKEND;
    }

    result = doca_kv_dev_create_attr_set_nguid(kv_attr, nguid_bytes);
    if (result != DOCA_SUCCESS) {
        NIXL_ERROR << "Failed to set DOCA KV device NGUID: " << doca_error_get_descr(result);
        return NIXL_ERR_BACKEND;
    }

    doca_kv_nvme_kernel_dev *raw_nvme_kvdev = nullptr;
    result = doca_kv_nvme_kernel_dev_create(attr.get(), &raw_nvme_kvdev);
    if (result != DOCA_SUCCESS) {
    ...
    kvdev_ = doca_kv_nvme_kernel_dev_as_kv_dev(nvmeKvdev_.get());
    if (!kvdev_) {
    ...
    result = doca_kv_dev_get_max_key_len(kvdev_, &dev_max_key_len);
    if (result != DOCA_SUCCESS) {
        NIXL_ERROR << "doca_kv_dev_get_max_key_len failed: " << doca_error_get_descr(result);
        cleanupDocaResources();
        return NIXL_ERR_BACKEND;
    }
    ...
    result = doca_kv_dev_get_max_value_len(kvdev_, &maxValueLen_);
    if (result != DOCA_SUCCESS) {
        NIXL_ERROR << "doca_kv_dev_get_max_value_len failed: " << doca_error_get_descr(result);
        cleanupDocaResources();
        return NIXL_ERR_BACKEND;
    }
