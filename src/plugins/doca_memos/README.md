...
A NIXL backend plugin that provides hardware-accelerated key-value storage operations using NVIDIA's DOCA KV library (`libdoca_kv`, headers `doca_kv_dev.h`, `doca_kv_io.h`, `doca_kv_nvme_kernel_dev.h`, `doca_kv_nvme_kernel_io.h`) for BlueField devices. The NIXL backend name is **DOCA_MEMOS**; the underlying DOCA package may be renamed in the future.
...
- **Libraries**:
  ...
  - `libdoca_kv` also provides the NVMe kernel device backend
...
|-----------|-------------|---------|
| `num_tasks` | Maximum number of concurrent tasks in the DOCA task pool (clamped to `doca_kv_io_cap_get_max_tasks()`) | `8192` |
| `nguid` | 32-character hex NVMe namespace NGUID passed to `doca_kv_dev_create_attr_set_nguid()` | all zeros |
...
- **Generic KV Device (kvdev)**: View of the NVMe kernel KV device through the `doca_kv_dev` API for capability queries
- **NVMe Kernel KV IO Context (kkvIo) / Generic KV IO Context (kvIo)**: Per-engine I/O context; created independently of any device, configured via `doca_kv_io_set_num_tasks()` and the per-operation `doca_kv_io_task_*_register_{completion,error}_cb()` functions before start, then bound to the device with `doca_kv_io_bind_dev()` once running
...
Tasks accepted during one scheduling pass are queued with `doca_task_submit_ex(..., DOCA_TASK_SUBMIT_FLAG_NONE)` and published with one `doca_ctx_flush_tasks()` call. Partial batches are flushed before the remaining request is queued for retry, avoiding a device submission system call per task without delaying accepted work.
