...
// via doca_kv_dev_get_max_key_len() during engine init.
...
private:
    // Custom deleter for nvmeKvdev_. A device is live from create to destroy, so
    // this only destroys. kvdev_ is a non-owning alias derived from nvmeKvdev_ and
    // is cleared before nvmeKvdev_.reset().
    struct NvmeKvdevDeleter {
        void
        operator()(doca_kv_nvme_kernel_dev *dev) const noexcept;
    };

    // The attribute object is only needed until the device is created, which copies
    // it. Scoping it to initDocaDevice() keeps every error path from having to
    // remember to destroy it.
    struct NvmeKvdevAttrDeleter {
        void
        operator()(doca_kv_nvme_kernel_dev_create_attr *attr) const noexcept;
    };

    doca_kv_dev *kvdev_ = nullptr;
    std::unique_ptr<doca_kv_nvme_kernel_dev, NvmeKvdevDeleter> nvmeKvdev_;
