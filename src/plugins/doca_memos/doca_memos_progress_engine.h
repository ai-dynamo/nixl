...
struct doca_kv_dev;
struct doca_kv_io;
struct doca_kv_io_task_store;
struct doca_kv_io_task_retrieve;
struct doca_kv_io_task_exist;
...
protected:
    nixlDocaMemosProgressEngine(struct doca_kv_nvme_kernel_dev *nvme_kvdev,
                                uint32_t num_tasks,
    ...
    bool
    trySubmitExistTask(nixlDocaMemosBackendReqH *req_h, bool &tasks_queued) const;
    void
    flushSubmittedTasks(bool tasks_queued) const;
    void
    ...
    taskCompletionCallback(struct doca_task *task,
                           ...
                           const struct doca_kv_io_task_retrieve *retrieve_task = nullptr);
    static void
    taskErrorCallback(struct doca_task *task,
                      union doca_data task_user_data,
                      union doca_data ctx_user_data);
    static void
    storeTaskCompletionCallback(struct doca_kv_io_task_store *task,
                                union doca_data task_user_data,
                                union doca_data ctx_user_data);
    static void
    storeTaskErrorCallback(struct doca_kv_io_task_store *task,
                           union doca_data task_user_data,
                           union doca_data ctx_user_data);
    static void
    retrieveTaskCompletionCallback(struct doca_kv_io_task_retrieve *task,
                                   union doca_data task_user_data,
                                   union doca_data ctx_user_data);
    static void
    retrieveTaskErrorCallback(struct doca_kv_io_task_retrieve *task,
                              union doca_data task_user_data,
                              union doca_data ctx_user_data);
    static void
    existTaskCompletionCallback(struct doca_kv_io_task_exist *task,
                                union doca_data task_user_data,
                                union doca_data ctx_user_data);
    static void
    existTaskErrorCallback(struct doca_kv_io_task_exist *task,
                           union doca_data task_user_data,
                           union doca_data ctx_user_data);
    static void
    ...
    // The single device this context serves. The API allows several per context;
    // one engine drives one device, so binding is static and set up before the
    // progress thread starts. Also names the device to unbind during cleanup.
    struct doca_kv_dev *boundDev_ = nullptr;
    struct doca_kv_nvme_kernel_io *kkvIo_ = nullptr;
    struct doca_kv_io *kvIo_ = nullptr;
...
public:
    nixlNoThreadProgressEngine(struct doca_kv_nvme_kernel_dev *nvme_kvdev,
                               uint32_t num_tasks,
                               uint32_t max_value_len);
 ...
 * doca_kv_io_task_*_alloc_init, doca_task_submit_ex, etc.). Bind and unbind are the
 * exception: they run in the base constructor and cleanup, before the thread starts
 * and after it joins.
 *
...
public:
    nixlThreadedProgressEngine(struct doca_kv_nvme_kernel_dev *nvme_kvdev,
                               uint32_t num_tasks,
                               uint32_t max_value_len,
