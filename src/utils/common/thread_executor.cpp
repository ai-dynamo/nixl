...
ThreadExecutor::executorLoop() {
    ...
    while (!stop_requested_.load()) {
        ...
        for (auto &[name, task_info] : tasks_) {
            ...
            if (now >= task_info->next_execution) {
                ...
                if (task_info->mode == TaskMode::ONESHOT) {
                    // Mark oneshot tasks for removal after execution
                    tasks_to_remove.push_back(name);
                } else {
        ...
        // Remove completed oneshot tasks and inactive tasks
        for (const auto &name : tasks_to_remove) {
            tasks_.erase(name);
        }

        // Execute tasks outside of lock to avoid blocking other operations
        lock.unlock();
        ...
        // If no tasks were executed, sleep for a short time
...
bool
ThreadExecutor::isTaskRegistered(const std::string &name) const {
    if (name.empty()) {
        return false;
    }
...
size_t
ThreadExecutor::getTaskCount() const {
    std::lock_guard<std::mutex> lock(tasks_mutex_);
    return tasks_.size();
}
