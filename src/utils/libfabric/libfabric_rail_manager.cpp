...
// loads backend custom string configuration item with possible env var override
static nixl_status_t
getCustomStringParam(const nixl_b_params_t &custom_params,
                     const std::string &key,
                     std::string &value);

// loads backend custom integer configuration item with possible env var override
static nixl_status_t
...
public:
    ...
    // load policy
    bool
    load(nixlLibfabricRailManager &rail_manager);
...
private:
    ...
    bool
    buildNumaDataRails(std::unique_ptr<nixlLibfabricTopology> &topology);
...
nixlLibfabricRailManager::init(const nixl_b_params_t &custom_params) {
    ...
    // in case no EFA device is found, we use defaul tpolicy
    ...
    // get bandwidth limit from user or compute it, then deduce rail limit
    size_t max_bw = 0;
    size_t max_rails = getDramRailLimit(custom_params, max_bw);
    ...
    if (max_rails == 0) {
        ...
        // rail count exceeds NUMA node limit = select round robin extended NUMA policy
        ...
        if (max_rails > numa_rail_count) {
            ...
            NIXL_WARN << "User-provided configuration value for max_bw_per_dram_mr (" << max_bw
        ...
        NIXL_WARN << "User-provided configuration value for max_bw_per_dram_mr (" << max_bw
                  << " Gbps) exceeds total machine capacity of " << total_nic_speed
                  << " Gbps, and will use all available rails";
        NIXL_WARN << "Using default (all) rail selection policy for DRAM memory type";
        dram_rail_selection_policy_ = std::make_unique<nixlLibfabricAllRailSelectionPolicy>();
    }

    // load policy
    if (!dram_rail_selection_policy_->load(*this)) {
        NIXL_ERROR << "Failed to load DRAM rail selection policy";
...
// helper to print rail ids to log
static std::ostream &
operator<<(std::ostream &os, const std::vector<size_t> &rail_ids) {
    os << "[";
...
size_t
nixlLibfabricRailManager::getDramRailLimit(const nixl_b_params_t &custom_params, size_t &max_bw) {
    ...
    size_t nic_speed = topology->getAvgNicBandwidth();
    if (nic_speed == 0) {
        NIXL_WARN << "Could not dedeuce average EFA device line bandwidth, NUMA-aware rail "
    ...
    if (nic_upstream_speed == 0) {
        NIXL_WARN << "Could not dedeuce average EFA device upstream link bandwidth, NUMA-aware "
                     "rail rail selection for DRAM memory type aborted";
        return 0;
    }
    ...
    (void)getCustomIntParam(custom_params, "max_bw_per_dram_seg", max_bw);
    if (max_bw == 0) {
        ...
        if (max_bw == 0) {
            NIXL_WARN << "Could not deduce average bandwidth limit per NUMA node, NUMA-aware rail "
                         "rail selection for DRAM type aborted";
            return 0;
        }
...
nixlLibfabricRailManager::selectRailsForMemory(void *mem_addr,
    ...
    if (mem_type == DRAM_SEG) {
        ...
        NIXL_TRACE << "Selected rails " << railIdsToString(selected_rails)
...
nixlLibfabricAllRailSelectionPolicy::selectRails(void *buffer,
    ...
    // select all rails
    selected_rails.resize(rail_count_);
    for (size_t i = 0; i < rail_count_; ++i) {
        selected_rails[i] = i;
    }
    return true;
}
...
nixlLibfabricNumaRailSelectionPolicy::selectSwitchRails(SwitchRailData &switch_rail_data,
    ...
    size_t max_rails = std::min(switch_rail_data.max_rails_, missing_rail_count);
...
nixlLibfabricNumaRailSelectionPolicy::selectExtraSwitchRail(SwitchRailData &switch_rail_data,
    ...
    for (size_t i = 0; i < switch_rail_count; ++i) {
        ...
        // TODO: if we really insist we can use bitset here for faster performance
...
bool
nixlLibfabricNumaRailSelectionPolicy::buildNumaDataRails(
    std::unique_ptr<nixlLibfabricTopology> &topology) {
    int numa_node_count = numa_num_configured_nodes();
    ...
    typedef std::unordered_map<uint8_t, SwitchData> SwitchMap;
    ...
    for (size_t i = 0; i < all_devices.size(); ++i) {
        ...
        int dev_numa_node_id = topology->getDeviceNumaNode(device);
        if (dev_numa_node_id < 0) {
            NIXL_WARN << "Failed to get NUMA node id for device " << device << ", skipping";
            continue;
        }
        if ((size_t)dev_numa_node_id >= numa_data_rails_.size()) {
        ...
        SwitchMap::iterator itr = switch_map.find(bus_id);
        if (itr == switch_map.end()) {
            size_t index = switch_map.size();
            itr =
                switch_map.insert(SwitchMap::value_type(bus_id, {index, switch_link_speed})).first;
        }
    ...
    for (size_t numa_node_id = 0; numa_node_id < numa_switch_map.size(); ++numa_node_id) {
        ...
        for (const auto &entry : switch_map) {
            ...
            NIXL_TRACE << "\t\tRails on PCIe switch on bus " << (int)entry.first << ": "
                       << railIdsToString(switch_data.rail_ids_);
        }
    }
    ...
    // TODO: is it possible to have nodes/switches without NICs attached?
    for (size_t i = 0; i < numa_data_rails_.size(); ++i) {
        ...
        for (size_t j = 0; j < numa_data_rails_[i].switch_data_.size(); ++j) {
            if (numa_data_rails_[i].switch_data_[j].rail_ids_.size() == 0) {
                NIXL_ERROR << "Failed to build data rail array for NUMA node " << i
                           << ", PCIe switche " << j << " has no data rails assigned";
                return false;
            }
            if (numa_data_rails_[i].switch_data_[j].max_rails_ == 0) {
                NIXL_ERROR << "Failed to build data rail array for NUMA node " << i
                           << ", PCIe switche " << j << " has no capacity for a single rail";
                return false;
            }
        }
    }
...
nixlLibfabricNumaRailSelectionPolicy::buildNumaDistanceMap() {
    ...
    size_t numa_node_count = (size_t)numa_num_configured_nodes();
    ...
    for (size_t i = 0; i < numa_node_count; ++i) {
        ...
        std::sort(
            numa_distance_map_[i].begin(), numa_distance_map_[i].end(), [i](int node1, int node2) {
                return numa_distance(node1, i) < numa_distance(node2, i);
            });
    }
}
