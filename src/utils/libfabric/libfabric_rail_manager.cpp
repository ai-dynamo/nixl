...
nixlLibfabricRailManager::nixlLibfabricRailManager(size_t striping_threshold)
    ...
    NIXL_DEBUG << "Creating rail manager with striping threshold: " << striping_threshold_ << " bytes";
    ...
    try {
        ...
        throw std::runtime_error("Topology discovery failed - cannot proceed without topology information");
    }
    ...
    nixl_status_t control_rail_status = createControlRails(all_efa_devices, NIXL_LIBFABRIC_DEFAULT_CONTROL_RAILS);
    if (control_rail_status != NIXL_SUCCESS) {
        throw std::runtime_error("Rail Manager failed to create control rails");
    }
    NIXL_DEBUG << "Successfully created " << data_rails_.size() << " data rails and " << control_rails_.size() << " control rails";
}
...
nixl_status_t nixlLibfabricRailManager::createDataRails(const std::vector<std::string>& efa_devices) {
    ...
    try {
        ...
        for (size_t i = 0; i < num_data_rails_; ++i) {
            data_rails_.emplace_back(std::make_unique<nixlLibfabricRail>(efa_devices[i], static_cast<uint16_t>(i)));
...
nixl_status_t nixlLibfabricRailManager::createControlRails(const std::vector<std::string>& efa_devices, size_t num_control_rails) {
    // Pre-allocate to ensure contiguous memory allocation
    ...
    try {
        ...
        for (size_t i = 0; i < num_control_rails_; ++i) {
            control_rails_.emplace_back(std::make_unique<nixlLibfabricRail>(efa_devices[i], static_cast<uint16_t>(i)));
...
bool nixlLibfabricRailManager::shouldUseStriping(size_t transfer_size) const {
    return transfer_size >= striping_threshold_;
}

nixl_status_t nixlLibfabricRailManager::prepareAndSubmitTransfer(
    ...
    if (!use_striping) {
        ...
        if (op_type == nixlLibfabricReq::WRITE) {
            uint64_t imm_data = NIXL_MAKE_IMM_DATA(NIXL_LIBFABRIC_MSG_TRANSFER, agent_idx, req->xfer_id);
            status = data_rails_[rail_id]->postWrite(
                req->local_addr, req->chunk_size, fi_mr_desc(req->local_mr),
                imm_data, dest_addrs[rail_id], req->remote_addr, req->remote_key, req);
        } else {
            status = data_rails_[rail_id]->postRead(
                req->local_addr, req->chunk_size, fi_mr_desc(req->local_mr),
                dest_addrs[rail_id], req->remote_addr, req->remote_key, req);
        }
        ...
        NIXL_DEBUG << "Round-robin: submitted single request on rail " << rail_id
                   << " for " << transfer_size << " bytes, XFER_ID: " << req->xfer_id;
...
std::vector<size_t> nixlLibfabricRailManager::selectRailsForMemory(void* mem_addr, nixl_mem_t mem_type) const {
    if (mem_type == VRAM_SEG) {
        ...
        if (gpu_rails.empty()) {
            NIXL_ERROR << "No valid rail mapping found for GPU " << gpu_id
                       << " (checked " << gpu_efa_devices.size() << " EFA devices)";
            return {};
        }

        NIXL_DEBUG << "VRAM memory " << mem_addr
                   << " on GPU " << gpu_id << " will use "
    ...
    if (mem_type == DRAM_SEG) {
        ...
        for (const std::string& efa_device : numa_efa_devices) {
            ...
            if (it != efa_device_to_rail_map.end()) {
                ...
                NIXL_DEBUG << "DRAM memory " << mem_addr
                           << " on NUMA node " << numa_node
                           << " mapped to rail " << it->second
                           << " (EFA device: " << efa_device << ")";
            }
        }
        ...
        NIXL_DEBUG << "DRAM memory " << mem_addr
                   << " on NUMA node " << numa_node << " will use "
...
nixl_status_t nixlLibfabricRailManager::registerMemory(void* buffer, size_t length, nixl_mem_t mem_type,
    ...
    nixl_status_t status = data_rails_[rail_idx]->registerMemory(
        ...
        NIXL_DEBUG << "Registered memory on rail " << rail_idx
                   << " (mr: " << reinterpret_cast<uintptr_t>(mr)
                   << ", key: " << key << ")";
    }
...
nixl_status_t nixlLibfabricRailManager::deregisterMemory(const std::vector<size_t>& selected_rails,
...
nixl_status_t nixlLibfabricRailManager::insertAllAddresses(RailType rail_type,
    ...
    if (endpoints.size() != rails.size()) {
        NIXL_ERROR << "Expected " << rails.size()
                   << " " << rail_type_str << " endpoints, got " << endpoints.size();
        return NIXL_ERR_INVALID_PARAM;
    }
    ...
    for (size_t rail_id = 0; rail_id < rails.size(); ++rail_id) {
        ...
        ep_names_out.push_back(rails[rail_id]->ep_name);  // This is char[LF_EP_NAME_MAX_LEN], will be converted to char*
...
nixl_status_t nixlLibfabricRailManager::cleanupConnection(RailType rail_type,
    ...
    if (fi_addrs_to_remove.size() != rails.size()) {
        NIXL_ERROR << "Expected " << rails.size()
                   << " " << rail_type_str << " fi_addrs, got " << fi_addrs_to_remove.size();
        return NIXL_ERR_INVALID_PARAM;
    }
    ...
    for (size_t rail_id = 0; rail_id < rails.size(); ++rail_id) {
        if (fi_addrs_to_remove[rail_id] != FI_ADDR_UNSPEC) {
            ...
            if (status != NIXL_SUCCESS) {
                NIXL_ERROR << "Failed to remove address from " << rail_type_str
                           << " rail " << rail_id << ", fi_addr: " << fi_addrs_to_remove[rail_id];
                overall_status = status;
                // Continue cleanup for other rails even if one fails
            } else {
                NIXL_DEBUG << "Successfully removed address from " << rail_type_str
                           << " rail " << rail_id << ", fi_addr: " << fi_addrs_to_remove[rail_id];
            }
        } else {
...
nixl_status_t nixlLibfabricRailManager::postControlMessage(ControlMessageType msg_type, const void* buf, size_t len,
                                                          fi_addr_t dest_addr, uint16_t agent_idx,
                                                          std::function<void()> completion_callback) {
    // Validation
    ...
    NIXL_TRACE << "Sending control message type " << msg_type_value
    ...
    if (status != NIXL_SUCCESS) {
        NIXL_ERROR << "Failed to send control message type "
                   << static_cast<int>(msg_type) << " on control rail " << control_rail_id;
...
nixl_status_t nixlLibfabricRailManager::progressActiveDataRails() {
    ...
    if (any_completions) {
        NIXL_TRACE << "Processed " << active_rails_.size() << " active rails with batching, completions found";
    }

    return any_completions ? NIXL_SUCCESS : NIXL_IN_PROG;
}

nixl_status_t nixlLibfabricRailManager::progressAllControlRails() {
    bool any_completions = false;
    for (size_t rail_id = 0; rail_id < num_control_rails_; ++rail_id) {
        nixl_status_t status = control_rails_[rail_id]->progressCompletionQueue(true);  // Blocking for control rails
        if (status == NIXL_SUCCESS) {
            any_completions = true;
...
nixl_status_t nixlLibfabricRailManager::validateAllRailsInitialized() {
...
struct fid_mr* nixlLibfabricRailManager::getMemoryDescriptor(size_t rail_id, struct fid_mr* mr) {
    if (rail_id >= data_rails_.size()) {
...
nixl_status_t nixlLibfabricRailManager::serializeMemoryKeys(const std::vector<uint64_t>& keys, void* buffer, std::string& str) const {
    nixlSerDes ser_des;
    ...
    NIXL_DEBUG << "Serialized memory keys for " << keys.size() << " rails"
               << " (buffer: " << buffer << ", size: " << str.length() << " bytes)";
    return NIXL_SUCCESS;
}

nixl_status_t nixlLibfabricRailManager::deserializeMemoryKeys(const std::string& serialized_data, std::vector<uint64_t>& keys_out, uint64_t& remote_addr_out) const {
    nixlSerDes ser_des;
...
nixl_status_t nixlLibfabricRailManager::serializeConnectionInfo(const std::string& user_prefix, std::string& str) const {
    ...
    str = ser_des.exportStr();
    NIXL_DEBUG << "Connection info serialized with prefix " << user_prefix << ", size: " << str.length();
    return NIXL_SUCCESS;
}

nixl_status_t nixlLibfabricRailManager::deserializeConnectionInfo(const std::string& user_prefix, const std::string& serialized_data,
    ...
    std::string data_prefix = user_prefix + "_data_ep_";
    std::string control_prefix = user_prefix + "_control_ep_";
    nixl_status_t data_status = deserializeRailEndpoints(ser_des, data_prefix, data_rails_.size(), data_endpoints_out);
    if (data_status != NIXL_SUCCESS) {
        NIXL_ERROR << "Failed to deserialize data rail endpoints with prefix: " << data_prefix;
        return data_status;
    }
    nixl_status_t control_status = deserializeRailEndpoints(ser_des, control_prefix, control_rails_.size(), control_endpoints_out);
    if (control_status != NIXL_SUCCESS) {
        NIXL_ERROR << "Failed to deserialize control rail endpoints with prefix: " << control_prefix;
        return control_status;
    }

    NIXL_DEBUG << "Connection info deserialized with prefix " << user_prefix << ": " << data_endpoints_out.size()
               << " data endpoints, " << control_endpoints_out.size() << " control endpoints";
    return NIXL_SUCCESS;
}
...
nixl_status_t nixlLibfabricRailManager::deserializeRailEndpoints(nixlSerDes& ser_des,
                                                                const std::string& key_prefix,
    ...
    for (size_t rail_id = 0; rail_id < expected_count; ++rail_id) {
        ...
        if (actual_len > (ssize_t)endpoints_out[rail_id].size()) {
            NIXL_ERROR << "Buffer too small for rail " << rail_id
                      << ", need " << actual_len << " bytes, have " << endpoints_out[rail_id].size();
            return NIXL_ERR_BACKEND;
        }
    ...
    NIXL_DEBUG << "Successfully deserialized " << expected_count
               << " rail endpoints";
    return NIXL_SUCCESS;
}

void nixlLibfabricRailManager::markRailActive(size_t rail_id) {
    if (rail_id >= data_rails_.size()) {
    ...
    if (was_inserted) {
        NIXL_DEBUG << "Marked rail " << rail_id << " as active (total active: " << active_rails_.size() << ")";
    } else {
        NIXL_TRACE << "Rail " << rail_id << " was already active";
    }
}

void nixlLibfabricRailManager::markRailInactive(size_t rail_id) {
    std::lock_guard<std::mutex> lock(active_rails_mutex_);
    size_t erased = active_rails_.erase(rail_id);
    if (erased > 0) {
        NIXL_DEBUG << "Marked rail " << rail_id << " as inactive (total active: " << active_rails_.size() << ")";
    } else {
        NIXL_TRACE << "Rail " << rail_id << " was not in active set";
    }
}

void nixlLibfabricRailManager::clearActiveRails() {
    std::lock_guard<std::mutex> lock(active_rails_mutex_);
...
size_t nixlLibfabricRailManager::getActiveRailCount() const {
    std::lock_guard<std::mutex> lock(active_rails_mutex_);
