...
nixlLibfabricTopology::getPcieDevData(const std::string &efa_device,
    ...
    if (itr == nic_info_map.end()) {
        ...
        NIXL_DEBUG << "EFA device " << efa_device << " has upstream link speed "
                   ...
                   << numa_node_id << " thourgh PCIe switch on bus id " << parent_switch_bus_id
    ...
    for (struct fi_info *cur = info; cur; cur = cur->next) {
        if (cur->domain_attr && cur->domain_attr->name && cur->nic && cur->nic->bus_attr) {
            ...
            if (cur->nic->bus_attr->bus_type == FI_BUS_PCI &&
                ...
                nic_speed_map[pcie_addr] = cur->nic->link_attr->speed;

                NIXL_TRACE << "Mapped PCIe " << pcie_address << " → Libfabric " << libfabric_name
                           << " (provider=" << provider_name
                           << ", upstream link speed: " << cur->nic->link_attr->speed << ")";
            }
        }
    }
...
nixlLibfabricTopology::buildTopologyAwareGrouping() {
    ...
    for (const auto &pair : pcie_to_libfabric_map) {
        ...
        if (hwloc_node) {
            ...
            // NOTE: speed is given in GB/s as float, we convert it to size_t Gbps
            ...
            nic.parent_switch_bus_id =
                getPcieDevParentSwitchBusId(hwloc_node, pcie_addr, nic.parent_switch_link_speed);
            if (nic.numa_node_id != INVALID_NUMA_NODE_ID) {
                ...
                NIXL_DEBUG << "EFA device " << libfabric_name << " mapped to NUMA node "
                           ...
                           << " Gbps, parent switch bus id: " << (int)nic.parent_switch_bus_id
                           << ")";
            }
...
uint8_t
nixlLibfabricTopology::getPcieDevParentSwitchBusId(hwloc_obj_t obj,
...
nixlLibfabricTopology::buildNumaSpeedMap() {
    ...
    // since topmost links may repeat themselves a few times, we need to accumulate per link bus id.
    // so we maintain a map of link bus id and accumulated speed with corresponding NUMA node
    typedef std::unordered_map<uint32_t, std::pair<float, uint16_t>> LinkSpeedMap;
    ...
    for (const auto &entry : nic_info_map) {
        ...
        if (hwloc_node != nullptr) {
            ...
            float topmost_speed = 0.0f;
            uint32_t topmost_bus_id = UINT32_MAX;
            while (itr != nullptr) {
                if (itr->type == HWLOC_OBJ_BRIDGE) {
                    // print info
            ...
            if (topmost_speed != 0.0f) {
                ...
                std::pair<LinkSpeedMap::iterator, bool> pairib = link_speed_map.insert(
                    LinkSpeedMap::value_type(topmost_bus_id, {topmost_speed, numa_node_id}));
                if (!pairib.second) {
    ...
    for (const auto &entry : link_speed_map) {
        ...
        numa_speed_map.resize(numa_node_id + 1, 0);
...
nixlLibfabricTopology::calcAvgNumaNodeBandwidth() {
    ...
    if (speed_uniform) {
        ...
        NIXL_DEBUG << "NUMA PCIe capacity is uniform accorss all nodes with link bandwidth: "
                   << avg_numa_speed << " Gbps";
    } else {
        avg_numa_speed = total_speed / speed_count;
        NIXL_WARN
            << "NUMA PCIe capacity is non-uniform accorss all nodes with average link bandwidth: "
...
nixlLibfabricTopology::calcAvgNicBandwidth() {
    ...
    for (const auto &entry : nic_info_map) {
        ...
        if (curr_speed != speed) {
            NIXL_WARN << "Non-uniform NIC " << entry.first << " speed: " << curr_speed << " Gbps";
            speed_uniform = false;
        }
        total_speed += curr_speed;
    }
    if (speed_uniform) {
        ...
        NIXL_WARN
            << "NIC bandwidth is non-uniform accorss all PCIe devices with average bandwidth: "
...
nixlLibfabricTopology::calcAvgNicUpstreamBandwidth() {
    ...
    if (speed_uniform) {
        ...
        NIXL_WARN << "NIC upstream link bandwidth is non-uniform accorss all PCIe devices with "
                     "average bandwidth: "
                  << avg_nic_speed << " Gbps, rail selection policy may be sub-optimal";
    }
}
