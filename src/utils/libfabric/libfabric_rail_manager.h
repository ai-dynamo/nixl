...
public:
    /** Initialize rail manager with topology discovery and create rails based on available EFA devices
     ...
     */
    nixl_status_t createDataRails(const std::vector<std::string>& efa_devices);
     ...
     * @return NIXL_SUCCESS on success, error code on failure
     */
    nixl_status_t createControlRails(const std::vector<std::string>& efa_devices, size_t num_control_rails);

    // Access rails
    /** Get reference to data rail by ID */
    nixlLibfabricRail& getDataRail(size_t rail_id) { return *data_rails_[rail_id]; }

    /** Get const reference to data rail by ID */
    const nixlLibfabricRail& getDataRail(size_t rail_id) const { return *data_rails_[rail_id]; }

    /** Get reference to control rail by ID */
    nixlLibfabricRail& getControlRail(size_t rail_id) { return *control_rails_[rail_id]; }

    /** Get const reference to control rail by ID */
    const nixlLibfabricRail& getControlRail(size_t rail_id) const { return *control_rails_[rail_id]; }

    /** Get total number of data rails */
    size_t getNumDataRails() const { return data_rails_.size(); }

    /** Get total number of control rails */
    size_t getNumControlRails() const { return control_rails_.size(); }
     ...
     * @return NIXL_SUCCESS on success, error code on failure
     */
    nixl_status_t registerMemory(void* buffer, size_t length, nixl_mem_t mem_type,
     ...
     * @return NIXL_SUCCESS on success, error code on failure
     */
    nixl_status_t deregisterMemory(const std::vector<size_t>& selected_rails,
     ...
     * @return NIXL_SUCCESS on success, error code on failure
     */
    nixl_status_t insertAllAddresses(RailType rail_type,
     ...
     * @return NIXL_SUCCESS on success, error code on failure
     */
    nixl_status_t cleanupConnection(RailType rail_type,
     ...
     * @return NIXL_SUCCESS on success, error code on failure
     */
    nixl_status_t prepareAndSubmitTransfer(
     ...
     * @return NIXL_SUCCESS on success, error code on failure
     */
    nixl_status_t postControlMessage(ControlMessageType msg_type, const void* buf, size_t len,
                                    fi_addr_t dest_addr, uint16_t agent_idx = 0,
    ...
    void markRailInactive(size_t rail_id);

    /** Clear all active rail markings */
    void clearActiveRails();

    /** Get count of currently active rails */
    size_t getActiveRailCount() const;

    // Memory Descriptor APIs
    /** Get memory descriptor for specified rail and MR */
    struct fid_mr* getMemoryDescriptor(size_t rail_id, struct fid_mr* mr);
     ...
     * @return NIXL_SUCCESS on success, error code on failure
     */
    nixl_status_t serializeMemoryKeys(const std::vector<uint64_t>& keys, void* buffer, std::string& str) const;
     ...
     * @return NIXL_SUCCESS on success, error code on failure
     */
    nixl_status_t deserializeMemoryKeys(const std::string& serialized_data, std::vector<uint64_t>& keys_out, uint64_t& remote_addr_out) const;
     ...
     * @return NIXL_SUCCESS on success, error code on failure
     */
    nixl_status_t serializeConnectionInfo(const std::string& user_prefix, std::string& str) const;
     ...
     * @return NIXL_SUCCESS on success, error code on failure
     */
    nixl_status_t deserializeConnectionInfo(const std::string& user_prefix, const std::string& serialized_data,
...
private:
    ...
    std::vector<size_t> selectRailsForMemory(void* mem_addr, nixl_mem_t mem_type) const;

    // Helper functions for connection SerDes
    void serializeRailEndpoints(nixlSerDes& ser_des, const std::string& key_prefix, RailType rail_type) const;
    nixl_status_t deserializeRailEndpoints(nixlSerDes& ser_des, const std::string& key_prefix,
                                          size_t expected_count, std::vector<std::array<char, LF_EP_NAME_MAX_LEN>>& endpoints_out) const;
