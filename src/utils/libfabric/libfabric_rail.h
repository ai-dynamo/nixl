...
struct nixlLibfabricReq {
    ...
    nixlLibfabricReq() : rail_id(0), xfer_id(0), buffer(nullptr), mr(nullptr),
                         buffer_size(0), operation_type(SEND), in_use(false),
                         chunk_offset(0), chunk_size(0), local_addr(nullptr),
...
public:
    ...
    virtual void release(nixlLibfabricReq* req);

    /** Find request by libfabric context pointer */
    nixlLibfabricReq* findByContext(void* context) const;
...
public:
    ...
    ControlRequestPool& operator=(const ControlRequestPool&) = delete;
    ControlRequestPool(ControlRequestPool&&) = delete;
    ControlRequestPool& operator=(ControlRequestPool&&) = delete;

    /** Initialize pool with buffers and pre-assigned XFER_IDs */
    nixl_status_t initializeWithBuffersAndXferIds(struct fid_domain* domain, const std::vector<uint32_t>& xfer_ids);

    /** Allocate control request with size validation */
    nixlLibfabricReq* allocate(size_t needed_size);
...
public:
    ...
    DataRequestPool& operator=(const DataRequestPool&) = delete;
    DataRequestPool(DataRequestPool&&) = delete;
    DataRequestPool& operator=(DataRequestPool&&) = delete;

    /** Initialize pool with pre-assigned XFER_IDs */
    nixl_status_t initializeWithXferIds(const std::vector<uint32_t>& xfer_ids);

    /** Allocate data request for specified operation type */
    nixlLibfabricReq* allocate(nixlLibfabricReq::OpType op_type);
};
...
inline std::ostream& operator<<(std::ostream& os, const ConnectionState& state) {
    switch (state) {
...
public:
    ...
    nixlLibfabricRail& operator=(const nixlLibfabricRail&) = delete;
    nixlLibfabricRail(nixlLibfabricRail&&) = delete;
    nixlLibfabricRail& operator=(nixlLibfabricRail&&) = delete;
    ...
    nixl_status_t registerMemory(void* buffer, size_t length, uint64_t access_flags,
                                 struct fid_mr** mr_out, uint64_t* key_out) const;

    /** Deregister memory from libfabric */
    nixl_status_t deregisterMemory(struct fid_mr* mr) const;

    // Address vector management methods
    /** Insert remote endpoint address into address vector */
    nixl_status_t insertAddress(const void* addr, fi_addr_t* fi_addr_out) const;

    /** Remove address from address vector */
    nixl_status_t removeAddress(fi_addr_t fi_addr) const;

    // Memory descriptor helper methods
    /** Get libfabric memory descriptor for MR */
    void* getMemoryDescriptor(struct fid_mr* mr) const;

    /** Get remote access key for MR */
    uint64_t getMemoryKey(struct fid_mr* mr) const;

    // Libfabric operation wrappers
    /** Post receive operation */
    nixl_status_t postRecv(nixlLibfabricReq* req) const;

    /** Post send operation with immediate data */
    nixl_status_t postSend(uint64_t immediate_data, fi_addr_t dest_addr, nixlLibfabricReq* req) const;

    /** Post RDMA write operation with immediate data */
    nixl_status_t postWrite(const void* local_buffer, size_t length, void* local_desc,
                            uint64_t immediate_data, fi_addr_t dest_addr,
                            uint64_t remote_addr, uint64_t remote_key, nixlLibfabricReq* req) const;

    /** Post RDMA read operation */
    nixl_status_t postRead(void* local_buffer, size_t length, void* local_desc,
    ...
    nixl_status_t progressCompletionQueue(bool use_blocking = false);
    ...
    void setConnectionAckCallback(std::function<void(uint16_t, nixlLibfabricConnection*, ConnectionState)> callback);

    /** Set callback for connection request processing */
    void setConnectionReqCallback(std::function<nixl_status_t(uint16_t, const std::string&, nixlLibfabricRail*)> callback);

    /** Set callback for XFER_ID tracking */
    void setXferIdCallback(std::function<void(uint32_t)> callback);

    // Optimized resource management methods
    /** Allocate control request with size validation */
    [[nodiscard]] nixlLibfabricReq* allocateControlRequest(size_t needed_size);

    /** Allocate data request for specified operation */
    [[nodiscard]] nixlLibfabricReq* allocateDataRequest(nixlLibfabricReq::OpType op_type);

    /** Release request back to appropriate pool */
    void releaseRequest(nixlLibfabricReq* req);

    /** Find request from libfabric context pointer */
    nixlLibfabricReq* findRequestFromContext(void* context) const;
...
private:
    ...
    std::function<nixl_status_t(uint16_t, const std::string&, nixlLibfabricRail*)> connectionReqCallback;
    // XFER_ID tracking callback
    std::function<void(uint32_t)> xferIdCallback;

    // Separate request pools for optimal performance
    ControlRequestPool control_request_pool_;  // 256 CONTROL requests (SEND/RECV) with internal buffers
    DataRequestPool data_request_pool_;        // 1024 DATA requests (WRITE/read) - no buffers needed

    // Configuration constants
    static constexpr size_t CONTROL_REQUESTS_PER_RAIL = 256;   // SEND/RECV operations (1:1 with buffers)
    static constexpr size_t DATA_REQUESTS_PER_RAIL = 1024;     // WRITE/read operations (no buffers)

    nixl_status_t processCompletionQueueEntry(struct fi_cq_data_entry* comp);
    nixl_status_t processLocalSendCompletion(struct fi_cq_data_entry* comp);
    nixl_status_t processLocalTransferCompletion(struct fi_cq_data_entry* comp, const char* operation_type);
    nixl_status_t processRecvCompletion(struct fi_cq_data_entry* comp);
    nixl_status_t processRemoteWriteCompletion(struct fi_cq_data_entry* comp) const;
};
