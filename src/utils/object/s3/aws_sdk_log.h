...
getNixlAwsLogLevel() {
    ...
    std::string lvl(env);
    for (auto &c : lvl) c = std::toupper(static_cast<unsigned char>(c));
...
public:
    ...
    void Log(Aws::Utils::Logging::LogLevel logLevel,
             const char *tag,
    ...
    void vaLog(Aws::Utils::Logging::LogLevel logLevel,
               const char *tag,
...
private:
    ...
    void dispatch(Aws::Utils::Logging::LogLevel logLevel,
                  const char *tag, const char *msg) {
        using Aws::Utils::Logging::LogLevel;
        switch (logLevel) {
            // Fatal -> ERROR: AWS-internal fatals must not abort the NIXL process
            case LogLevel::Fatal: NIXL_ERROR << "[AWS:" << tag << "] " << msg; break;
            case LogLevel::Error: NIXL_ERROR << "[AWS:" << tag << "] " << msg; break;
            case LogLevel::Warn:  NIXL_WARN  << "[AWS:" << tag << "] " << msg; break;
            case LogLevel::Info:  NIXL_INFO  << "[AWS:" << tag << "] " << msg; break;
            case LogLevel::Debug: NIXL_DEBUG << "[AWS:" << tag << "] " << msg; break;
            case LogLevel::Trace: NIXL_TRACE << "[AWS:" << tag << "] " << msg; break;
            default: break;
        }
    }
};
